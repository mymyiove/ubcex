// ═══════════════════════════════════════════════════════════
// highlight.js — 하이라이트 강의 캐러셀 + 직무 선택 반영
// ═══════════════════════════════════════════════════════════
let popularCourses = [];
let curatorPicks = [];
let popularIndex = 0;
...
function initHighlight() {
  // ★ 개선: 선택된 직무가 있으면 해당 직무 키워드로 필터링
  let baseCourses = S.courses;
  
  if (S.selectedFamilies && S.selectedFamilies.length > 0) {
    // 선택된 직무의 모든 role prompt 키워드 수집
    const jobKeywords = [];
    S.selectedFamilies.forEach(famId => {
      const fam = CURATION[famId];
      if (fam && fam.roles) {
        fam.roles.forEach(role => {
          role.prompt.split(/\s+/).forEach(kw => {
            if (kw.length > 2) jobKeywords.push(kw.toLowerCase());
          });
        });
      }
    });
    ...
    if (jobKeywords.length > 0) {
      // 직무 관련 강의 필터링 (제목, 카테고리, 주제에 키워드 포함)
      const jobFiltered = baseCourses.filter(c => {
        const text = `${c.title} ${c.category} ${c.topic} ${c.headline}`.toLowerCase();
        return jobKeywords.some(kw => text.includes(kw));
      });
      
      // 직무 관련 강의가 충분하면 그걸 사용, 아니면 전체에서
      if (jobFiltered.length >= 6) baseCourses = jobFiltered;
    }
  }
  
  // ★ 최신 인기 별 — 한글자막 + 평점 4.5+ + 수강 1000+ + 최신순
  popularCourses = baseCourses
  ...
  // 직무 필터링 후 6개 미만이면 전체에서 보충
  if (popularCourses.length < 6) {
    const fallback = S.courses
      .filter(c => hasKoreanSub(c) && c.rating >= 4.5 && c.enrollments >= 1000)
      .filter(c => !popularCourses.find(p => p.id === c.id))
      .sort((a, b) => {
        const da = a.lastUpdated ? new Date(a.lastUpdated) : new Date(0);
        const db = b.lastUpdated ? new Date(b.lastUpdated) : new Date(0);
        return db - da;
      })
      .slice(0, 6 - popularCourses.length);
    popularCourses = [...popularCourses, ...fallback];
  }

  // ★ 항해사's PICK — 큐레이터 설정 기반
...
function renderPopularCarousel() {
    ...
    <div class="highlight-card" style="--card-color:${color}">
      ...
      <div class="highlight-content">
        ...
        <div class="highlight-meta">
          ...
          👥 ${enrollText}
          ⏱️ ${formatDuration(course.contentLength) || '-'}
          ${course.isNew ? '✨신규' : ''}
          🇰🇷 한글자막
        </div>
  ...
  if (titleEl) titleEl.addEventListener('click', () => openSidePanel(course));
  const detailBtn = document.querySelector(`#popular-carousel .highlight-detail-btn[data-course-id="${course.id}"]`);
  if (detailBtn) detailBtn.addEventListener('click', () => openSidePanel(course));
}
...
function startAutoRotation() {
  ...
  popularInterval = setInterval(() => {
    if (popularCourses.length > 0) { popularIndex = (popularIndex + 1) % popularCourses.length; renderPopularCarousel(); }
  }, 3000);
  curatorInterval = setInterval(() => {
    if (curatorPicks.length > 0) { curatorIndex = (curatorIndex + 1) % curatorPicks.length; renderCuratorCarousel(); }
  }, 3000);
}
...
function manageCuratorPicks() {
  ...
  if (newPicks !== null) {
    ...
    curatorIndex = 0;
    renderCuratorCarousel();
    toast(`⭐ 항해사's PICK이 ${curatorPicks.length}개로 업데이트되었습니다.`);
  }
}
