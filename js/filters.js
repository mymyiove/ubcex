...
function initMultiSelects() {
  ...
  populateMS('f-duration', [
    {v:'60',l:'⏱️ 1시간 미만'},{v:'120',l:'⏱️ 1-2시간'},
    {v:'180',l:'⏱️ 2-3시간'},{v:'240',l:'⏱️ 3-4시간'},
    {v:'300',l:'⏱️ 4시간 이상'}
  ]);
  populateMS('f-score', [
    {v:'100',l:'🎯 100점 이상'},{v:'200',l:'🎯 200점 이상'},
    {v:'300',l:'🎯 300점 이상'}
  ]);
  populateMS('f-updated', [
    {v:'1m',l:'📅 1개월 이내'},{v:'3m',l:'📅 3개월 이내'},
    {v:'1y',l:'📅 1년 이내'},{v:'2y',l:'📅 2년 이내'},
    {v:'3y',l:'📅 3년 이내'}
  ]);
}
...
function updateMSBtn(fid) {
  ...
  var defaults = {
    ...
    'f-subtitles':'모든 자막', 'f-duration':'모든 시간', 'f-score':'모든 점수',
    'f-updated':'모든 기간'
  };
...
function translateKeywords(keywords) {
  ...
  for (var i = 0; i < keywords.length; i++) {
    ...
    if (mapped) {
      for (var j = 0; j < mapped.length; j++) translated.push(mapped[j]);
    }
    var entries = Object.entries(KO_EN_MAP);
    for (var j = 0; j < entries.length; j++) {
      for (var k = 0; k < entries[j][1].length; k++) {
        if (entries[j][1][k].toLowerCase() === kw.toLowerCase() && translated.indexOf(entries[j][0]) === -1) {
          translated.push(entries[j][0]);
        }
      }
    }
  }
...
function scoreWithSensitivity(course, keywords, sensitivity, originalKeywords) {
  ...
  for (var i = 0; i < keywords.length; i++) {
    ...
    var isOrig = false;
    for (var j = 0; j < transOrig.length; j++) {
      if (transOrig[j].toLowerCase() === kwl) { isOrig = true; break; }
    }
    if (isOrig) continue;
...
function _doApplyFilters() {
  ...
  if (cats.length > 0) {
    filtered = filtered.filter(function(c) {
      for (var i = 0; i < cats.length; i++) {
        if (c.category && c.category.indexOf(cats[i]) !== -1) return true;
      }
      return false;
    });
  }
  ...
  if (diffs.length > 0) {
    filtered = filtered.filter(function(c) {
      for (var i = 0; i < diffs.length; i++) {
        if (c.difficulty && c.difficulty.toUpperCase() === diffs[i].toUpperCase()) return true;
      }
      return false;
    });
  }
  ...
  if (subs.length > 0) {
    filtered = filtered.filter(function(c) {
      ...
      for (var i = 0; i < subs.length; i++) {
        if (c.subtitles.toLowerCase().indexOf(subs[i].toLowerCase()) !== -1) return true;
      }
      return false;
    });
  }
  ...
  if (updatedFilters.length > 0) {
    ...
    if (earliestCutoff) {
      filtered = filtered.filter(function(c) {
        ...
        return new Date(c.lastUpdated) >= earliestCutoff;
      });
    }
  }
  ...
  if (searchText) {
    ...
    filtered = filtered.filter(function(c) {
      return filterWithSensitivity(c, keywords, sensitivity, originalKeywords);
    });
    ...
    if (config.minScore > 0) {
      filtered = filtered.filter(function(c) { return c._score >= config.minScore; });
    }
    if (scores.length > 0) {
      filtered = filtered.filter(function(c) {
        for (var i = 0; i < scores.length; i++) {
          if (c._score >= parseInt(scores[i])) return true;
        }
        return false;
      });
    }
  ...
  switch (sort) {
    ...
    case 'latest': filtered.sort(function(a, b) {
      return (b.lastUpdated ? new Date(b.lastUpdated) : new Date(0)) - (a.lastUpdated ? new Date(a.lastUpdated) : new Date(0));
    }); break;
    case 'alpha': filtered.sort(function(a, b) { return (a.title || '').localeCompare(b.title || ''); }); break;
  }

  // 스마트 결과 제한: 검색어 있을 때 1000개 이상이면 상위 100개만
