// ═══════════════════════════════════════════════════════════
// filters.js — 추천도 로직 개선 + 감도 기반 필터링
// ═══════════════════════════════════════════════════════════
...
// ★ 개선: 스코어링 — 원본 키워드 vs 확장 키워드 가중치 분리
function scoreWithSensitivity(course, keywords, sensitivity, originalKeywords) {
  ...
  keywords.forEach(kw => {
    ...
    const multiplier = isOriginal ? 1.0 : 0.4;
    
    const titleLower = (course.title || '').toLowerCase();
    const hasSpaces = kw.includes(' ');
    
    if (hasSpaces) {
      if (titleLower.includes(kwl)) totalScore += Math.round(config.scoreWeights.title * multiplier);
    } else {
      const wordRegex = new RegExp(`\\b${kwl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
      if (wordRegex.test(course.title || '')) totalScore += Math.round(config.scoreWeights.title * multiplier);
      else if (titleLower.includes(kwl)) totalScore += Math.round(config.scoreWeights.title * 0.7 * multiplier);
    }
    
    if (config.scoreWeights.category > 0 && (course.category||'').toLowerCase().includes(kwl)) totalScore += Math.round(config.scoreWeights.category * multiplier);
    if (config.scoreWeights.topic > 0 && (course.topic||'').toLowerCase().includes(kwl)) totalScore += Math.round(config.scoreWeights.topic * multiplier);
    if (config.scoreWeights.headline > 0 && (course.headline||'').toLowerCase().includes(kwl)) totalScore += Math.round(config.scoreWeights.headline * multiplier);
    if (config.scoreWeights.objectives > 0 && (course.objectives||'').toLowerCase().includes(kwl)) totalScore += Math.round(config.scoreWeights.objectives * multiplier);
    if (config.scoreWeights.description > 0 && (course.description||'').toLowerCase().includes(kwl)) totalScore += Math.round(config.scoreWeights.description * multiplier);
  });
...
function filterWithSensitivity(course, keywords, sensitivity, originalKeywords) {
  ...
  const filterKeywords = originalKeywords && originalKeywords.length > 0 ? originalKeywords : keywords;
  
  if (S.searchMode === 'and') return filterKeywords.every(kw => searchText.includes(kw.toLowerCase()));
  else return filterKeywords.some(kw => searchText.includes(kw.toLowerCase()));
}

// ★ 개선: applyFilters — 원본 키워드 별도 보관
function applyFilters() {
  ...
  if(searchText) {
    let originalKeywords = searchText.split(/[,\s]+/).map(k=>k.trim().toLowerCase()).filter(k=>k.length>0);
...
function renderActiveFilters() {
  ...
  if(q) html+=`<span class="filter-chip">검색: ${q} (${S.searchMode.toUpperCase()}) <button class="filter-chip-x" data-fid="search">×</button></span>`;
