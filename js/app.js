...
function getExcludedCourses() { try { return JSON.parse(localStorage.getItem('excluded_courses') || '[]'); } catch { return []; } }
function getCuratorPicks() { try { return JSON.parse(localStorage.getItem('curator_picks') || '["8324","1717020","2360128"]'); } catch { return ['8324','1717020','2360128']; } }

const S = {
  subdomain: '', courses: [], filtered: [], selectedFamilies: [],
  selectedIds: new Set(), page: 1, rows: 15,
  searchMode: 'OR', sensitivity: 'normal', viewMode: 'table',
  sortField: 'score', sortDir: 'desc',
  highlightTimers: {}, highlightPaused: {},
};
...
const FIELD_WEIGHTS = { title: 50, category: 30, topic: 25, headline: 15, objectives: 10, description: 5 };
...
function getCatColor(cat) {
  ...
  if (!CAT_COLORS[cat]) CAT_COLORS[cat] = CAT_COLOR_PALETTE[Object.keys(CAT_COLORS).length % CAT_COLOR_PALETTE.length];
  return CAT_COLORS[cat];
}
function getCatEmoji(cat) {
  if (!cat) return '📁';
  for (const [key, emoji] of Object.entries(CAT_EMOJIS)) { if (cat.toLowerCase().includes(key.toLowerCase())) return emoji; }
  return '📁';
}
function buildCourseUrl(course) { return `https://${S.subdomain}.udemy.com/course/${course.slug || course.id}/`; }
...
function isNewCourse(dateStr) {
  ...
  try { return (new Date() - new Date(dateStr)) < 90 * 24 * 60 * 60 * 1000; } catch { return false; }
}
...
async function launch() {
  ...
  const warp = $('#warp-overlay'); warp.classList.add('active');
  ...
  $('#launch-emoji').textContent = '🌟'; $('#launch-message').textContent = '워프 점프!'; $('#progress-fill').style.width = '100%';
  await new Promise(r => setTimeout(r, 600));
  warp.classList.remove('active'); $('#gate-page').style.display = 'none'; $('#app-container').style.display = 'block';
  initApp();
...
async function playLaunchSequence() {
  for (let i = 0; i < LAUNCH_STEPS.length; i++) {
    ...
    $('#launch-emoji').textContent = step.emoji; $('#launch-message').textContent = step.message;
...
function initApp() {
  ...
  renderDashCards(); initFilterOptions(); initMultiSelects();
  initHighlight(); setupHoverPause();

  // ★ 수정 #6: 헤더 제목 클릭 → 맨위로 + 3번 클릭 큐레이터 PICK
  let clickCount = 0;
  ...
  $('#search-input')?.addEventListener('input', () => {
    clearTimeout(debounceTimer); clearTimeout(suggestionTimer);
    debounceTimer = setTimeout(applyFilters, 300);
    suggestionTimer = setTimeout(() => showSearchSuggestions($('#search-input').value.trim()), 200);
  });
  $('#search-input')?.addEventListener('focus', () => { const v=$('#search-input').value.trim(); if(v.length>=2) showSearchSuggestions(v); });
  document.addEventListener('click', e => { if(!e.target.closest('.scanner-input-wrap')) $('#search-suggestions')?.classList.remove('open'); });

  // 검색 모드
  $$('.scan-mode-btn[data-mode]').forEach(btn => btn.addEventListener('click', () => { $$('.scan-mode-btn[data-mode]').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); S.searchMode=btn.dataset.mode; applyFilters(); }));
  // 감도
  $$('.sensitivity-btn').forEach(btn => btn.addEventListener('click', () => { $$('.sensitivity-btn').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); S.sensitivity=btn.dataset.sensitivity; applyFilters(); toast(`🎚️ 감도: ${SENSITIVITY_CONFIG[S.sensitivity]?.label}`); }));
  ...
  // 필터 토글
  $('#btn-filter-toggle')?.addEventListener('click', () => { $('#filters-grid')?.classList.toggle('open'); $('#btn-filter-toggle')?.classList.toggle('active'); });
  ...
  ['f-level','f-duration','f-score','f-attr'].forEach(id => $(`#${id}`)?.addEventListener('change', applyFilters));

  // CSV, 공유, 리셋
  $('#btn-csv')?.addEventListener('click', () => showCSVModal(false));
  $('#btn-csv2')?.addEventListener('click', () => showCSVModal(false));
  $('#btn-share')?.addEventListener('click', shareLink);
  $('#btn-reset-inline')?.addEventListener('click', () => resetAll(true));
  $('#stat-csv')?.addEventListener('click', () => showCSVModal(false));

  // 뷰 모드
  $$('.view-btn[data-view]').forEach(btn => btn.addEventListener('click', () => { $$('.view-btn[data-view]').forEach(b=>b.classList.remove('active')); btn.classList.add('active'); S.viewMode=btn.dataset.view; renderList(); }));
  ...
  $('#csv-select-default')?.addEventListener('click', () => { const d=['title','category','instructor','rating','enrollments','language','subtitles','level','contentLength','updated','url']; $$('#csv-columns input').forEach(cb=>cb.checked=d.includes(cb.value)); });

  // 미션 센터 + TOP 별
  initMissionCenter(); initStars();
  ...
  document.addEventListener('keydown', e => {
    ...
    if(!e.target.matches('input,textarea,select')){if(e.key==='ArrowLeft'&&S.page>1){S.page--;renderList();}if(e.key==='ArrowRight'){const tp=Math.ceil(S.filtered.length/S.rows);if(S.page<tp){S.page++;renderList();}}}
  });
  ...
  document.addEventListener('click', e => {
    ...
    if(S.sortField===field) S.sortDir=S.sortDir==='desc'?'asc':'desc'; else { S.sortField=field; S.sortDir='desc'; }
    $$('th.sortable').forEach(t=>t.classList.remove('sort-asc','sort-desc'));
    th.classList.add(S.sortDir==='desc'?'sort-desc':'sort-asc');
    $('#sort-select').value=field; applyFilters();
  });

  applyURLParams(); applyFilters();
}

function switchTab(tabId) {
  $$('.tab-panel').forEach(p=>p.classList.remove('active'));
  $(`#panel-${tabId}`)?.classList.add('active');
  const ls=$('#list-section'); if(ls) ls.style.display=tabId==='stars'?'none':'';
}
...
async function handleAIScan() {
  ...
  const btn=$('#btn-ai-scan'); const orig=btn.textContent;
  btn.disabled=true; btn.innerHTML='<span class="spinner"></span> 분석 중...';
  try {
    let aiKw=null;
    try { const res=await fetch('/api/ai-expand',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query,context:'udemy course search'})}); const data=await res.json(); if(data.success&&data.keywords){aiKw=data.keywords;toast('🤖 Gemini AI가 항로를 계산했습니다!');} } catch(e){console.warn('Gemini 실패:',e);}
    if(!aiKw){aiKw=localKeywordExpand(query);toast('🤖 로컬 AI 모드로 키워드를 확장했습니다.','warning');}
    renderAIPanel(aiKw,query);
  } catch(e){console.error(e);toast('AI 내비게이션 오류','error');} finally{btn.disabled=false;btn.textContent=orig;}
}

function localKeywordExpand(query) {
  const kws=query.split(/[,;]+/).map(k=>k.trim().toLowerCase()).filter(Boolean);
  const g={original:{label:'🎯 원본 키워드',keywords:[...kws]},translated:{label:'🌐 번역 확장',keywords:[]},related:{label:'🔗 관련 키워드',keywords:[]},broader:{label:'🔭 광역 확장',keywords:[]}};
  kws.forEach(kw=>{
    Object.entries(KO_EN_MAP).forEach(([ko,enList])=>{if(ko.includes(kw)||kw.includes(ko))enList.forEach(en=>{if(!g.translated.keywords.includes(en)&&!kws.includes(en))g.translated.keywords.push(en);});enList.forEach(en=>{if(en.includes(kw)||kw.includes(en)){if(!g.translated.keywords.includes(ko)&&!kws.includes(ko))g.translated.keywords.push(ko);}});});
    if(RELATED_MAP[kw])RELATED_MAP[kw].forEach(r=>{if(!g.related.keywords.includes(r)&&!kws.includes(r))g.related.keywords.push(r);});
    Object.entries(RELATED_MAP).forEach(([key,vals])=>{if(key.includes(kw)||kw.includes(key))vals.slice(0,3).forEach(r=>{if(!g.broader.keywords.includes(r)&&!kws.includes(r)&&!g.related.keywords.includes(r))g.broader.keywords.push(r);});});
    Object.entries(CURATION).forEach(([id,data])=>{if(data.name.includes(kw)||data.keywords.some(k=>k.includes(kw)||kw.includes(k)))data.keywords.forEach(k=>{if(!g.broader.keywords.includes(k)&&!kws.includes(k)&&!g.related.keywords.includes(k))g.broader.keywords.push(k);});});
  });
  g.translated.keywords=[...new Set(g.translated.keywords)].slice(0,8);
  g.related.keywords=[...new Set(g.related.keywords)].slice(0,10);
  g.broader.keywords=[...new Set(g.broader.keywords)].slice(0,6);
  return g;
}

function renderAIPanel(aiKw,origQuery) {
  const panel=$('#ai-panel'),results=$('#ai-panel-results'); if(!panel||!results) return;
  let html='';
  if(aiKw&&typeof aiKw==='object'&&!Array.isArray(aiKw)){Object.entries(aiKw).forEach(([gk,gr])=>{const label=gr.label||gk;const kws=gr.keywords||(Array.isArray(gr)?gr:[]);if(kws.length===0)return;html+=`<div class="ai-kw-group"><div class="group-label">${label}</div><div class="ai-kw-tags">${kws.map(kw=>`<span class="ai-kw-tag selected" data-kw="${kw}">${kw}</span>`).join('')}</div></div>`;});}
  else if(Array.isArray(aiKw)){html+=`<div class="ai-kw-group"><div class="group-label">🤖 AI 확장</div><div class="ai-kw-tags">${aiKw.map(kw=>`<span class="ai-kw-tag selected" data-kw="${kw}">${kw}</span>`).join('')}</div></div>`;}
  if(!html) html='<p style="color:var(--text-muted);font-size:0.85rem;">확장할 키워드를 찾지 못했습니다.</p>';
  const allKws=extractAllKeywords(aiKw);
  const pc=S.courses.filter(c=>{const t=`${c.title} ${c.category} ${c.topic} ${c.headline}`.toLowerCase();return allKws.some(kw=>t.includes(kw.toLowerCase()));}).length;
  html+=`<div id="ai-preview-count" style="margin-top:0.8rem;font-size:0.8rem;color:var(--text-muted);">💡 약 <strong style="color:var(--accent-light)">${pc.toLocaleString()}</strong>개 매칭</div>`;
  results.innerHTML=html;
  results.querySelectorAll('.ai-kw-tag').forEach(tag=>tag.addEventListener('click',()=>{tag.classList.toggle('selected');updateAIPreviewCount();}));
  panel.classList.add('open');
  const inp=$('#search-input'); if(inp){inp.classList.add('glow-enhanced');setTimeout(()=>inp.classList.remove('glow-enhanced'),3000);}
}
function updateAIPreviewCount(){const sel=$$('#ai-panel-results .ai-kw-tag.selected').map(t=>t.dataset.kw).filter(Boolean);const c=S.courses.filter(c=>{const t=`${c.title} ${c.category} ${c.topic} ${c.headline}`.toLowerCase();return sel.some(kw=>t.includes(kw.toLowerCase()));}).length;const el=$('#ai-preview-count strong');if(el)el.textContent=c.toLocaleString();}
function extractAllKeywords(aiKw){const a=[];if(Array.isArray(aiKw))return aiKw;if(aiKw&&typeof aiKw==='object')Object.values(aiKw).forEach(g=>{const kws=g.keywords||(Array.isArray(g)?g:[]);kws.forEach(kw=>{if(!a.includes(kw))a.push(kw);});});return a;}
function applyAIKeywords(){const sel=$$('#ai-panel-results .ai-kw-tag.selected').map(t=>t.dataset.kw).filter(Boolean);if(sel.length===0){toast('키워드를 선택해주세요.','warning');return;}$('#search-input').value=sel.join(', ');$('#ai-panel').classList.remove('open');applyFilters();toast(`🚀 ${sel.length}개 키워드로 스캔!`);}

// ═══ 스마트 검색 제안 ═══
function showSearchSuggestions(query) {
  const container=$('#search-suggestions'); if(!container) return;
  if(query.length<2){container.classList.remove('open');return;}
  const ql=query.toLowerCase();
  const tm=S.courses.filter(c=>c.title?.toLowerCase().includes(ql)).sort((a,b)=>(b.enrollments||0)-(a.enrollments||0)).slice(0,3);
  const cm=[...new Set(S.courses.filter(c=>c.category?.toLowerCase().includes(ql)).map(c=>c.category?.split(',')[0]?.trim()))].filter(Boolean).slice(0,2);
  const tpm=[...new Set(S.courses.filter(c=>c.topic?.toLowerCase().includes(ql)).map(c=>c.topic))].filter(Boolean).slice(0,2);
  const koEn=[];Object.entries(KO_EN_MAP).forEach(([ko,enList])=>{if(ko.includes(ql)||enList.some(en=>en.includes(ql)))koEn.push({ko,en:enList[0]});});
  let html='';
  if(tm.length>0){html+=`<div class="suggestion-category">🔥 인기 강의</div>`;html+=tm.map(c=>`<div class="suggestion-item" data-course-id="${c.id}"><span class="suggestion-icon">📖</span><span class="suggestion-text">${c.title.length>50?c.title.substring(0,50)+'...':c.title}</span><span class="suggestion-badge">⭐${c.rating?.toFixed(1)||'-'}</span></div>`).join('');}
  if(cm.length>0){html+=`<div class="suggestion-category">🌌 카테고리</div>`;html+=cm.map(cat=>`<div class="suggestion-item" data-category="${cat}"><span class="suggestion-icon">${getCatEmoji(cat)}</span><span class="suggestion-text">${cat}</span><span class="suggestion-badge">카테고리 필터</span></div>`).join('');}
  if(tpm.length>0){html+=`<div class="suggestion-category">💡 주제</div>`;html+=tpm.map(topic=>`<div class="suggestion-item" data-topic="${topic}"><span class="suggestion-icon">🏷️</span><span class="suggestion-text">${topic}</span></div>`).join('');}
  if(koEn.length>0){html+=`<div class="suggestion-category">🌐 번역 제안</div>`;html+=koEn.slice(0,2).map(s=>`<div class="suggestion-item" data-keyword="${s.en}"><span class="suggestion-icon">🔄</span><span class="suggestion-text">${s.ko} → ${s.en}</span></div>`).join('');}
  if(!html){container.classList.remove('open');return;}
  container.innerHTML=html; container.classList.add('open');
  container.querySelectorAll('.suggestion-item').forEach(item=>{item.addEventListener('click',()=>{
...
function renderDashCards() {
  const total=S.courses.length, kr=S.courses.filter(c=>c.hasKoreanSub).length, newC=S.courses.filter(c=>c.isNew).length, cats=new Set(S.courses.map(c=>c.category).filter(Boolean)).size;
  $('#stat-total-val').textContent=total.toLocaleString(); $('#stat-kr-val').textContent=kr.toLocaleString();
  $('#stat-new-val').textContent=newC.toLocaleString(); $('#stat-cat-val').textContent=cats.toLocaleString();
}
function initFilterOptions() {
  const dur=$('#f-duration'); if(dur&&dur.options.length<=1)[['short','~3시간'],['medium','3~10시간'],['long','10~20시간'],['vlong','20시간+']].forEach(([v,t])=>{const o=document.createElement('option');o.value=v;o.textContent=t;dur.appendChild(o);});
  const sc=$('#f-score'); if(sc&&sc.options.length<=1)[['high','70점+'],['mid','40~69점'],['low','~39점']].forEach(([v,t])=>{const o=document.createElement('option');o.value=v;o.textContent=t;sc.appendChild(o);});
  const at=$('#f-attr'); if(at&&at.options.length<=1)[['new','✨ 신규'],['korean','🇰🇷 한국어자막'],['highrated','⭐ 4.5+']].forEach(([v,t])=>{const o=document.createElement('option');o.value=v;o.textContent=t;at.appendChild(o);});
  const lv=$('#f-level'); if(lv&&lv.options.length<=1){const levels=[...new Set(S.courses.map(c=>c.level).filter(Boolean))].sort();levels.forEach(l=>{const o=document.createElement('option');o.value=l;o.textContent=l;lv.appendChild(o);});}
}
function initMultiSelects() {
  initMS('f-category',[...new Set(S.courses.map(c=>c.category).filter(Boolean))].sort());
  initMS('f-lang',[...new Set(S.courses.map(c=>c.language).filter(Boolean))].sort());
  const allSubs=new Set(); S.courses.forEach(c=>{if(c.subtitles)c.subtitles.split(',').forEach(s=>{const t=s.trim();if(t)allSubs.add(t);});}); initMS('f-subtitle',[...allSubs].sort());
}
function initMS(id,items) {
  ...
  btn.addEventListener('click',e=>{e.stopPropagation();$$('.ms-panel').forEach(p=>{if(p!==panel)p.classList.remove('open');});panel.classList.toggle('open');});
  panel.innerHTML=items.map(item=>`<div class="ms-item"><label><input type="checkbox" value="${item}"> ${item}</label></div>`).join('');
  panel.querySelectorAll('input').forEach(cb=>cb.addEventListener('change',()=>{const sel=[...panel.querySelectorAll('input:checked')].map(i=>i.value);btn.textContent=sel.length>0?`${sel.length}개 선택`:btn.dataset.default;applyFilters();}));
}
function getMSValues(id){const w=$(`#${id}`);if(!w)return[];return[...w.querySelectorAll('.ms-panel input:checked')].map(i=>i.value);}
function setMSValues(id,values){const w=$(`#${id}`);if(!w)return;w.querySelectorAll('.ms-panel input').forEach(cb=>{cb.checked=values.includes(cb.value);});const btn=w.querySelector('.ms-btn');btn.textContent=values.length>0?`${values.length}개 선택`:btn.dataset.default;}
function removeMSValue(id,value){const w=$(`#${id}`);if(!w)return;const cb=w.querySelector(`.ms-panel input[value="${value}"]`);if(cb){cb.checked=false;const btn=w.querySelector('.ms-btn');const sel=[...w.querySelectorAll('.ms-panel input:checked')].map(i=>i.value);btn.textContent=sel.length>0?`${sel.length}개 선택`:btn.dataset.default;}applyFilters();}
...
function applyFilters() {
  ...
  keywords.forEach(kw=>{Object.entries(KO_EN_MAP).forEach(([ko,enList])=>{if(ko.includes(kw))enList.forEach(en=>{if(!expanded.includes(en))expanded.push(en);});enList.forEach(en=>{if(en.includes(kw)&&!expanded.includes(ko))expanded.push(ko);});});});

  const catF=getMSValues('f-category'),langF=getMSValues('f-lang'),subF=getMSValues('f-subtitle');
  const lvF=$('#f-level')?.value||'',durF=$('#f-duration')?.value||'',scoreF=$('#f-score')?.value||'',attrF=$('#f-attr')?.value||'';
  const fields=SENSITIVITY_CONFIG[S.sensitivity]?.fields||SENSITIVITY_CONFIG.normal.fields;

  let results=S.courses.filter(c=>{
    if(catF.length>0&&!catF.includes(c.category))return false;
    if(langF.length>0&&!langF.includes(c.language))return false;
    if(subF.length>0&&!subF.some(s=>c.subtitles?.includes(s)))return false;
    if(lvF&&c.level!==lvF)return false;
    if(durF){const h=c.contentLength||0;if(durF==='short'&&h>3)return false;if(durF==='medium'&&(h<=3||h>10))return false;if(durF==='long'&&(h<=10||h>20))return false;if(durF==='vlong'&&h<=20)return false;}
    if(attrF==='new'&&!c.isNew)return false;
    if(attrF==='korean'&&!c.hasKoreanSub)return false;
    if(attrF==='highrated'&&c.rating<4.5)return false;
    if(expanded.length===0)return true;
    const st=fields.map(f=>(c[f]||'').toLowerCase()).join(' ');
    return S.searchMode==='AND'?expanded.every(kw=>st.includes(kw)):expanded.some(kw=>st.includes(kw));
  });

  // 스코어링
  if(expanded.length>0){
    results.forEach(c=>{let score=0;expanded.forEach(kw=>{fields.forEach(f=>{if((c[f]||'').toLowerCase().includes(kw))score+=FIELD_WEIGHTS[f]||5;});});if(c.isNew)score+=3;if(c.hasKoreanSub)score+=3;if(c.rating>=4.5)score+=5;c.score=score;});
    // ★ 수정 #7: 정규화 0~100
    const maxScore=Math.max(...results.map(c=>c.score),1);
    results.forEach(c=>{c.displayScore=Math.round((c.score/maxScore)*100);});
  } else { results.forEach(c=>{c.score=0;c.displayScore=0;}); }

  if(scoreF){results=results.filter(c=>{if(scoreF==='high')return c.displayScore>=70;if(scoreF==='mid')return c.displayScore>=40&&c.displayScore<70;if(scoreF==='low')return c.displayScore<40;return true;});}

  const sf=$('#sort-select')?.value||S.sortField||'score', sd=S.sortDir||'desc';
  results.sort((a,b)=>{let va=a[sf==='score'?'displayScore':sf],vb=b[sf==='score'?'displayScore':sf];if(sf==='title')return sd==='asc'?(va||'').localeCompare(vb||''):(vb||'').localeCompare(va||'');va=parseFloat(va)||0;vb=parseFloat(vb)||0;return sd==='desc'?vb-va:va-vb;});

  S.filtered=results; S.page=1;
  $('#results-count').textContent=results.length.toLocaleString();
  renderActiveFilters(catF,langF,subF,lvF,durF,scoreF,attrF,query);
  renderList();
}

function renderActiveFilters(catF,langF,subF,lvF,durF,scoreF,attrF,query) {
  const c=$('#active-filters'); if(!c) return; let h='';
  if(query) h+=`<span class="filter-chip">🔍 ${query} <button class="filter-chip-x" onclick="$('#search-input').value='';applyFilters();">×</button></span>`;
  catF.forEach(v=>h+=`<span class="filter-chip">🌌 ${v} <button class="filter-chip-x" onclick="removeMSValue('f-category','${v}')">×</button></span>`);
  langF.forEach(v=>h+=`<span class="filter-chip">🌐 ${v} <button class="filter-chip-x" onclick="removeMSValue('f-lang','${v}')">×</button></span>`);
  subF.forEach(v=>h+=`<span class="filter-chip">💬 ${v} <button class="filter-chip-x" onclick="removeMSValue('f-subtitle','${v}')">×</button></span>`);
  if(lvF) h+=`<span class="filter-chip">📊 ${lvF} <button class="filter-chip-x" onclick="$('#f-level').value='';applyFilters();">×</button></span>`;
  if(durF) h+=`<span class="filter-chip">⏱️ ${$('#f-duration').selectedOptions[0]?.text} <button class="filter-chip-x" onclick="$('#f-duration').value='';applyFilters();">×</button></span>`;
  if(scoreF) h+=`<span class="filter-chip">🎯 ${$('#f-score').selectedOptions[0]?.text} <button class="filter-chip-x" onclick="$('#f-score').value='';applyFilters();">×</button></span>`;
  if(attrF) h+=`<span class="filter-chip">✨ ${$('#f-attr').selectedOptions[0]?.text} <button class="filter-chip-x" onclick="$('#f-attr').value='';applyFilters();">×</button></span>`;
  c.innerHTML=h;
}

// ═══ 렌더링 ═══
function getPageData(){return S.filtered.slice((S.page-1)*S.rows,(S.page-1)*S.rows+S.rows);}

function renderList() {
  const data=getPageData(), tbody=$('#course-tbody'), cardGrid=$('#card-grid'), compactList=$('#compact-list');
  if(tbody){
    tbody.innerHTML=data.length>0?data.map(c=>{
      const cc=getCatColor(c.category), sc=c.displayScore>=70?'score-high':c.displayScore>=40?'score-mid':'score-low';
      return `<tr style="--row-cat-color:${cc}">
        <td class="col-check"><input type="checkbox" ${S.selectedIds.has(c.id)?'checked':''} onchange="toggleSelect('${c.id}',this.checked)"></td>
        <td class="td-score"><span class="score-badge ${sc}">${c.displayScore||'-'}</span></td>
        <td><span class="cat-badge" style="border-color:${cc}">${getCatEmoji(c.category)} ${c.category}</span></td>
  ...
  if(cardGrid){cardGrid.innerHTML=data.map((c,i)=>{const cc=getCatColor(c.category);return `<div class="course-card" style="animation-delay:${i*0.05}s" onclick="openSidePanelById('${c.id}')"><div class="card-cat-stripe" style="background:${cc}"></div><h4>${c.title}</h4><div class="card-meta">${getCatEmoji(c.category)} ${c.category} · ⭐${c.rating?.toFixed(1)||'-'} · 👥▲${(c.enrollments||0).toLocaleString()}</div><div class="card-tags">${c.hasKoreanSub?'<span class="cat-badge" style="border-color:var(--success);color:var(--success)">🇰🇷</span>':''}${c.isNew?'<span class="badge-new">NEW</span>':''}<span class="cat-badge">${c.level||'-'}</span></div><a href="${buildCourseUrl(c)}" target="_blank" class="card-cta" onclick="event.stopPropagation()">수강하기 →</a></div>`;}).join('');}
  if(compactList){compactList.innerHTML=data.map(c=>`<div class="compact-item"><input type="checkbox" ${S.selectedIds.has(c.id)?'checked':''} onchange="toggleSelect('${c.id}',this.checked)" style="accent-color:var(--accent)"><span class="compact-title"><a onclick="openSidePanelById('${c.id}')">${c.title}</a></span><span style="font-size:0.75rem;color:var(--text-muted)">⭐${c.rating?.toFixed(1)||'-'}</span></div>`).join('');}
  const tw=$('#table-wrap'); if(tw) tw.style.display=S.viewMode==='table'?'':'none';
  ...
  renderPagination(); updateFAB();
}

function renderPagination(){const c=$('#pagination');if(!c)return;const tp=Math.ceil(S.filtered.length/S.rows);if(tp<=1){c.innerHTML='';return;}let h=`<button ${S.page<=1?'disabled':''} onclick="goPage(${S.page-1})">◀</button>`;const r=2;for(let i=1;i<=tp;i++){if(i===1||i===tp||(i>=S.page-r&&i<=S.page+r))h+=`<button class="${i===S.page?'active':''}" onclick="goPage(${i})">${i}</button>`;else if(i===S.page-r-1||i===S.page+r+1)h+=`<button disabled>…</button>`;}h+=`<button ${S.page>=tp?'disabled':''} onclick="goPage(${S.page+1})">▶</button>`;c.innerHTML=h;}
function goPage(p){S.page=p;renderList();window.scrollTo({top:($('#list-section')?.offsetTop||0)-80,behavior:'smooth'});}
function updateFAB(){const f=$('#fab');const c=S.selectedIds.size;if(f){f.classList.toggle('visible',c>0);const fc=$('#fab-count');if(fc)fc.textContent=c;}}
...
function openSidePanel(course) {
  const panel=$('#side-panel'),overlay=$('#side-panel-overlay'),content=$('#sp-content');
  const url=buildCourseUrl(course), cc=getCatColor(course.category);
  const objHTML=course.objectivesArr&&course.objectivesArr.length>0?`<div class="sp-section sp-translatable"><h4>🎯 학습 목표</h4><ul>${course.objectivesArr.map(o=>`<li>${o.trim()}</li>`).join('')}</ul></div>`:'';
  content.innerHTML=`
    ...
    <div class="sp-meta">
      <span class="sp-meta-tag" style="border-color:${cc}">${getCatEmoji(course.category)} ${course.category}</span>
...
function closeSidePanel(){$('#side-panel')?.classList.remove('open');$('#side-panel-overlay')?.classList.remove('open');}

// ★ 수정 #4: 사이드바 번역
async function translateSidePanel(btn) {
  if(!btn) return;
  if(btn.classList.contains('translated')){$$('#sp-content .translated-text').forEach(el=>el.remove());btn.textContent='🌐 한국어로 번역하기';btn.classList.remove('translated');return;}
  ...
  for(const el of targets){
    const text=el.textContent.trim(); if(!text||text.length<5) continue;
    if((text.match(/[가-힣]/g)||[]).length>text.length*0.3) continue;
    try{const res=await fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=ko&dt=t&q=${encodeURIComponent(text.substring(0,2000))}`);const data=await res.json();const tr=data[0]?.map(s=>s[0]).join('')||'';if(tr&&tr!==text){const div=document.createElement('div');div.className='translated-text';div.textContent=tr;el.after(div);translated++;}}catch(e){console.warn('번역 실패:',e);}
...
function findSimilar(courseId){const c=S.courses.find(x=>x.id===courseId);if(!c)return;closeSidePanel();$('#search-input').value=c.title.split(/[\s:—\-,]+/).filter(w=>w.length>2).slice(0,3).join(', ');applyFilters();toast(`🔍 유사 강의 검색`);}

// ═══ 하이라이트 ═══
function initHighlight() {
  const trending=S.courses.filter(c=>c.isNew||c.enrollments>1000).sort((a,b)=>(b.enrollments||0)-(a.enrollments||0)).slice(0,6);
  const pickIds=getCuratorPicks(); const picks=pickIds.map(id=>S.courses.find(c=>c.id===id)).filter(Boolean);
...
function renderHighlightCarousel(type,courses) {
  const carousel=$(`#${type}-carousel`),indicator=$(`#${type}-indicator`);
  if(!carousel||courses.length===0) return;
  let idx=0;
  function render(){const c=courses[idx];if(!c)return;const cc=getCatColor(c.category);carousel.innerHTML=`<div class="highlight-card ${type==='picks'?'curator-pick':''}" style="--card-color:${cc}">${c.image?`<div class="highlight-thumbnail"><img src="${c.image}" alt="" loading="lazy"></div>`:''}<div class="highlight-content"><div class="highlight-category" style="color:${cc}">${getCatEmoji(c.category)} ${c.category}</div><h4 class="highlight-title" onclick="openSidePanelById('${c.id}')">${c.title}</h4><p class="highlight-desc">${c.headline||''}</p><div class="highlight-meta"><span>⭐ ${c.rating?.toFixed(1)||'-'}</span><span>👥 ▲${(c.enrollments||0).toLocaleString()}</span>${c.hasKoreanSub?'<span>🇰🇷</span>':''}</div><div class="highlight-actions"><a href="${buildCourseUrl(c)}" target="_blank" class="highlight-cta">수강하기</a><button class="highlight-detail-btn" onclick="openSidePanelById('${c.id}')">상세보기</button></div></div></div>`;if(indicator)indicator.textContent=`${idx+1} / ${courses.length}`;}
  render();
  $$(`.highlight-prev[data-target="${type}"]`).forEach(btn=>btn.addEventListener('click',()=>{idx=(idx-1+courses.length)%courses.length;render();}));
  $$(`.highlight-next[data-target="${type}"]`).forEach(btn=>btn.addEventListener('click',()=>{idx=(idx+1)%courses.length;render();}));
  const interval=type==='trending'?8000:10000;
  S.highlightTimers[type]=setInterval(()=>{if(!S.highlightPaused[type]){idx=(idx+1)%courses.length;render();}},interval);
}
function setupHoverPause(){['trending','picks'].forEach(type=>{const p=$(`#highlight-${type}`);if(p){p.addEventListener('mouseenter',()=>{S.highlightPaused[type]=true;});p.addEventListener('mouseleave',()=>{S.highlightPaused[type]=false;});}});}

// ═══ 미션 센터 ═══
function initMissionCenter() {
  const grid=$('#galaxy-grid'),orbitGrid=$('#orbit-grid'); if(!grid) return;
  Object.entries(CURATION).forEach(([id,data])=>{const card=document.createElement('div');card.className='galaxy-card';card.dataset.family=id;card.innerHTML=`<span class="emoji">${data.emoji}</span><span class="label">${data.name}</span>`;card.addEventListener('click',()=>{$$('.galaxy-card').forEach(c=>c.classList.remove('active'));card.classList.add('active');renderOrbits(id);});grid.appendChild(card);});
  $$('#level-bar input[name="level"]').forEach(radio=>radio.addEventListener('change',()=>{const ag=$('.galaxy-card.active');if(ag)renderOrbits(ag.dataset.family);}));
  $('#mission-search-input')?.addEventListener('input',()=>{const ag=$('.galaxy-card.active');if(ag)renderOrbits(ag.dataset.family);});
}
function renderOrbits(familyId) {
  const og=$('#orbit-grid'); if(!og) return;
  const family=CURATION[familyId]; if(!family) return;
  const lvF=document.querySelector('#level-bar input[name="level"]:checked')?.value||'';
  const sq=($('#mission-search-input')?.value||'').toLowerCase();
  let courses=S.courses.filter(c=>{const t=`${c.title} ${c.category} ${c.topic} ${c.headline}`.toLowerCase();return family.keywords.some(kw=>t.includes(kw));});
  if(lvF) courses=courses.filter(c=>c.level===lvF);
  if(sq) courses=courses.filter(c=>c.title.toLowerCase().includes(sq)||c.topic?.toLowerCase().includes(sq));
  courses.sort((a,b)=>(b.rating||0)-(a.rating||0)); courses=courses.slice(0,20);
  og.innerHTML=courses.length>0?courses.map((c,i)=>`<div class="orbit-card" style="animation-delay:${i*0.05}s" onclick="openSidePanelById('${c.id}')"><h4>${getCatEmoji(c.category)} ${c.title.length>40?c.title.substring(0,40)+'...':c.title}</h4><div class="orbit-tags"><span class="cat-badge">${c.level||'-'}</span><span class="cat-badge">⭐${c.rating?.toFixed(1)||'-'}</span>${c.hasKoreanSub?'<span class="cat-badge" style="border-color:var(--success);color:var(--success)">🇰🇷</span>':''}</div></div>`).join(''):'<div class="empty-state"><p>해당 조건의 강의가 없습니다.</p></div>';
}

// ═══ TOP 별 ═══
function initStars(){const t=$('#stars-toggle');if(!t)return;t.querySelectorAll('button').forEach(btn=>btn.addEventListener('click',()=>{t.querySelectorAll('button').forEach(b=>b.classList.remove('active'));btn.classList.add('active');renderStars(btn.dataset.stars);}));renderStars('rating');}
function renderStars(mode) {
  const hero=$('#stars-hero'),grid=$('#stars-grid'); if(!hero||!grid) return;
  let courses;
  if(mode==='korean') courses=S.courses.filter(c=>c.hasKoreanSub).sort((a,b)=>(b.rating||0)-(a.rating||0));
  else if(mode==='enrollments') courses=[...S.courses].sort((a,b)=>(b.enrollments||0)-(a.enrollments||0));
  else courses=S.courses.filter(c=>c.enrollments>100).sort((a,b)=>(b.rating||0)-(a.rating||0));
  const top=courses[0], rest=courses.slice(1,13);
...
// ═══ CSV + 공유 + 리셋 + URL ═══
function showCSVModal(selectedOnly){S._csvSelectedOnly=selectedOnly;$('#csv-modal')?.classList.add('active');}
function executeCSVDownload(){
  const data=S._csvSelectedOnly?S.courses.filter(c=>S.selectedIds.has(c.id)):S.filtered;
  if(data.length===0){toast('다운로드할 데이터가 없습니다.','warning');return;}
  const cols=$$('#csv-columns input:checked').map(cb=>cb.value);if(cols.length===0){toast('최소 1개 컬럼을 선택해주세요.','warning');return;}
  const names={id:'강의ID',title:'강의명',category:'카테고리',instructor:'강사',rating:'평점',enrollments:'수강신청수',language:'언어',subtitles:'자막',level:'난이도',contentLength:'강의시간',updated:'업데이트',url:'강의링크'};
  const header=cols.map(c=>names[c]||c).join(',');
  const rows=data.map(c=>cols.map(col=>{let v=col==='url'?buildCourseUrl(c):(c[col]||'');v=String(v).replace(/"/g,'""');if(v.includes(',')||v.includes('"')||v.includes('\n'))v=`"${v}"`;return v;}).join(','));
  const csv='\uFEFF'+header+'\n'+rows.join('\n');
  const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});const url=URL.createObjectURL(blob);
  const a=document.createElement('a');a.href=url;a.download=`udemy_courses_${S.subdomain}_${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(url);
  $('#csv-modal')?.classList.remove('active');toast(`📥 ${data.length}개 CSV 다운로드!`);
}
function shareLink(){const q=$('#search-input')?.value?.trim()||'';const url=new URL(window.location.href);if(q)url.searchParams.set('q',q);url.searchParams.set('sub',S.subdomain);navigator.clipboard.writeText(url.toString());toast('🔗 공유 링크 복사!');}
function applyURLParams(){const p=new URLSearchParams(window.location.search);const q=p.get('q');if(q)$('#search-input').value=q;}
function resetAll(showToast){$('#search-input').value='';$('#f-level').value='';$('#f-duration').value='';$('#f-score').value='';$('#f-attr').value='';$$('.ms-panel input').forEach(cb=>cb.checked=false);$$('.ms-btn').forEach(btn=>btn.textContent=btn.dataset.default||'전체');$('#sort-select').value='score';S.selectedIds.clear();S.page=1;S.sortField='score';S.sortDir='desc';$$('th.sortable').forEach(t=>t.classList.remove('sort-asc','sort-desc'));applyFilters();if(showToast)toast('✨ 초기화 완료!');}
