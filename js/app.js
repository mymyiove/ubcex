...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (saved && saved !== ADMIN_CODE) {
    ...
    setTimeout(function() { var ql = document.getElementById('quick-launch'); if (ql) ql.addEventListener('click', function() { document.getElementById('input-subdomain').value = saved; goStep2(); }); }, 0);
  }
  ...
  if (previewEl) { previewEl.style.cursor = 'pointer'; previewEl.addEventListener('click', function() { var sub = document.getElementById('input-subdomain').value.trim(); if (sub && sub !== ADMIN_CODE) window.open('https://' + sub + '.udemy.com', '_blank'); }); }
  ...
  for (var i = 0; i < curationEntries.length; i++) { (function(id, data) { var card = document.createElement('div'); card.className = 'gate-job-card'; card.dataset.family = id; card.innerHTML = '<span class="emoji">' + data.emoji + '</span><span class="label">' + data.name + '</span>'; card.addEventListener('click', function() { card.classList.add('selected'); launch(); }); grid.appendChild(card); })(curationEntries[i][0], curationEntries[i][1]); }
  ...
  var scrollBtn = document.getElementById('scroll-to-top'); if (scrollBtn) scrollBtn.addEventListener('click', function() { window.scrollTo({ top: 0, behavior: 'smooth' }); });
  var gateClickCount = 0;
  var gateLabel = document.querySelector('.gate-input-group label');
  if (gateLabel) { gateLabel.addEventListener('click', function() { gateClickCount++; setTimeout(function() { gateClickCount = 0; }, 1000); if (gateClickCount === 3) { enterAdminMode(); gateClickCount = 0; } }); }
});

function goStep2() { var sub = document.getElementById('input-subdomain').value.trim(); if (!sub) { toast('모선 주소를 입력해주세요.', 'error'); return; } if (sub === ADMIN_CODE) { enterAdminMode(); return; } S.subdomain = sub; localStorage.setItem('kv_subdomain', sub); document.getElementById('gate-step-1').classList.remove('active'); document.getElementById('gate-step-2').classList.add('active'); }
...
function enterAdminMode() {
  ...
  var existingApp = document.getElementById('app-container'); if (existingApp) existingApp.style.display = 'none';
  var footer = document.querySelector('.app-footer');
  var ap = document.createElement('div'); ap.id = 'admin-panel';
  ap.innerHTML = '<div class="admin-container">' +
    ...
    '</div></div>';
  if (footer) document.body.insertBefore(ap, footer); else document.body.appendChild(ap);
  ...
  if (btnThumbSearch) btnThumbSearch.addEventListener('click', function() {
    ...
    for (var i = 0; i < uids.length; i++) {
      var uid = uids[i]; var course = null;
  ...
  if (btnThumbDownloadAll) btnThumbDownloadAll.addEventListener('click', function() {
    ...
    if (imgs.length === 0) { toast('다운로드할 썸네일이 없습니다.', 'warning'); return; }
    if (typeof JSZip === 'undefined') { toast('⚠️ ZIP 라이브러리 로딩 중... 잠시 후 다시 시도해주세요.', 'warning'); return; }
    var zip = new JSZip();
    var log = document.getElementById('thumb-log');
    if (log) log.innerHTML = '<div class="log-entry log-info">📦 ZIP 생성 중... (0/' + imgs.length + ')</div>';
    btnThumbDownloadAll.disabled = true; btnThumbDownloadAll.textContent = '⏳ ZIP 생성 중...';
    var promises = [];
...
async function launch() {
  ...
  var warp = document.getElementById('warp-overlay'); warp.classList.add('active'); var loadingPromise = playLaunchSequence(); var dataLoaded = false;
  try { var totalChunks = 0; try { var r = await fetch(WORKER_URL+'/status',{headers:{'Authorization':'Bearer '+WORKER_SECRET}}); var s = await r.json(); totalChunks = s.totalChunks||0; } catch(e) {} var allCourses = [];
    if (totalChunks > 0) { var m = document.getElementById('launch-message'); if (m) m.textContent = '병렬 로딩 중... ('+totalChunks+' chunks)'; var promises = []; for (var i = 0; i < totalChunks; i++) { (function(idx){ promises.push(fetch(WORKER_URL+'/get-courses?chunk='+idx,{headers:{'Authorization':'Bearer '+WORKER_SECRET}}).then(function(r){return r.ok?r.json():[];}).catch(function(){return[];})); })(i); } var results = await Promise.all(promises); for (var i = 0; i < results.length; i++) { if (Array.isArray(results[i]) && results[i].length > 0) allCourses = allCourses.concat(results[i]); } if (m) m.textContent = allCourses.length.toLocaleString()+'개 로드 완료!'; }
    else { var ci = 0; while (true) { try { var r2 = await fetch(WORKER_URL+'/get-courses?chunk='+ci,{headers:{'Authorization':'Bearer '+WORKER_SECRET}}); if (!r2.ok) break; var d2 = await r2.json(); if (!d2||!Array.isArray(d2)||d2.length===0) break; allCourses = allCourses.concat(d2); ci++; if (ci>=50) break; } catch(e) { break; } } }
    var excluded = getExcludedCourses(); if (excluded.length > 0) allCourses = allCourses.filter(function(c) { return excluded.indexOf(c.id) === -1; });
    if (allCourses.length > 0) { S.courses = processCourses(allCourses); dataLoaded = true; }
  } catch (e) { console.error(e); }
  await loadingPromise; if (!dataLoaded) { toast('강의 데이터를 불러올 수 없습니다.', 'error'); warp.classList.remove('active'); return; }
  document.getElementById('launch-emoji').textContent = '🌟'; document.getElementById('launch-message').textContent = '워프 점프!'; document.getElementById('progress-fill').style.width = '100%';
  await new Promise(function(r) { setTimeout(r, 600); }); warp.classList.remove('active'); document.getElementById('gate-page').style.display = 'none'; document.getElementById('app-container').style.display = 'block'; initApp(); toast('🌌 '+S.subdomain+' 우주에 도착! '+S.courses.length.toLocaleString()+'개의 별');
}
async function playLaunchSequence() { for (var i = 0; i < LAUNCH_STEPS.length; i++) { document.getElementById('launch-emoji').textContent = LAUNCH_STEPS[i].emoji; document.getElementById('launch-message').textContent = LAUNCH_STEPS[i].message; document.getElementById('progress-fill').style.width = ((i+1)/(LAUNCH_STEPS.length+1)*100)+'%'; await new Promise(function(r) { setTimeout(r, 600+Math.random()*400); }); } }
...
function initApp() {
  ...
  var navBtns = document.querySelectorAll('.header-nav-btn'); for (var i = 0; i < navBtns.length; i++) { (function(btn) { btn.addEventListener('click', function() { for (var j = 0; j < navBtns.length; j++) navBtns[j].classList.remove('active'); btn.classList.add('active'); switchTab(btn.dataset.tab); }); })(navBtns[i]); }
  var searchInput = document.getElementById('search-input'); var debounceTimer;
  if (searchInput) { searchInput.addEventListener('input', function() { clearTimeout(debounceTimer); debounceTimer = setTimeout(function() { showSearchSuggestions(searchInput.value.trim()); }, 250); }); searchInput.addEventListener('focus', function() { if (searchInput.value.trim().length >= 2) showSearchSuggestions(searchInput.value.trim()); }); searchInput.addEventListener('keyup', function(e) { if (e.key === 'Enter') { var sg = document.getElementById('search-suggestions'); if (sg) sg.classList.remove('open'); clearTimeout(debounceTimer); S.showAllResults = false; applyFilters(); } }); }
  ...
  var modeBtns = document.querySelectorAll('.scan-mode-btn[data-mode]'); for (var i = 0; i < modeBtns.length; i++) { (function(btn) { btn.addEventListener('click', function() { for (var j = 0; j < modeBtns.length; j++) modeBtns[j].classList.remove('active'); btn.classList.add('active'); S.searchMode = btn.dataset.mode; S.showAllResults = false; applyFilters(); }); })(modeBtns[i]); }
  var sensBtns = document.querySelectorAll('.sensitivity-btn'); for (var i = 0; i < sensBtns.length; i++) { (function(btn) { btn.addEventListener('click', function() { for (var j = 0; j < sensBtns.length; j++) sensBtns[j].classList.remove('active'); btn.classList.add('active'); S.sensitivity = btn.dataset.sensitivity; S.showAllResults = false; applyFilters(); toast('🎚️ 감도: '+(SENSITIVITY_CONFIG[S.sensitivity]?SENSITIVITY_CONFIG[S.sensitivity].label:'')); }); })(sensBtns[i]); }
  ...
  var viewBtns = document.querySelectorAll('.view-btn'); for (var i = 0; i < viewBtns.length; i++) { (function(btn) { btn.addEventListener('click', function() { for (var j = 0; j < viewBtns.length; j++) viewBtns[j].classList.remove('active'); btn.classList.add('active'); if (btn.dataset.view) { S.viewMode = btn.dataset.view; renderList(); } }); })(viewBtns[i]); }
  var checkAll = document.getElementById('check-all'); if (checkAll) checkAll.addEventListener('change', function() { var ids = getPageData().map(function(c) { return c.id; }); for (var i = 0; i < ids.length; i++) { if (checkAll.checked) S.selectedIds.add(ids[i]); else S.selectedIds.delete(ids[i]); } renderList(); updateFAB(); });
  ...
  applyURLParams();
  if (S.selectedFamilies && S.selectedFamilies.length > 0 && !document.getElementById('search-input').value.trim()) { var jobKeywords = []; for (var fi = 0; fi < S.selectedFamilies.length; fi++) { var famData = CURATION[S.selectedFamilies[fi]]; if (famData) { var nameWords = famData.name.split(/[\/·,\s]+/); for (var ni = 0; ni < nameWords.length; ni++) { var w = nameWords[ni].trim(); if (w.length >= 2 && jobKeywords.indexOf(w) === -1) jobKeywords.push(w); } } } if (jobKeywords.length > 0) { document.getElementById('search-input').value = jobKeywords.join(', '); S.searchMode = 'or'; var mbs = document.querySelectorAll('.scan-mode-btn[data-mode]'); for (var mi = 0; mi < mbs.length; mi++) { mbs[mi].classList.remove('active'); if (mbs[mi].getAttribute('data-mode') === 'or') mbs[mi].classList.add('active'); } } }
  applyFilters();
  var clickCount = 0; var headerTitle = document.getElementById('header-title');
  if (headerTitle) headerTitle.addEventListener('click', function() { window.scrollTo({ top: 0, behavior: 'smooth' }); clickCount++; setTimeout(function() { clickCount = 0; }, 1000); if (clickCount === 3) { document.getElementById('app-container').style.display = 'none'; enterAdminMode(); clickCount = 0; } });
  ...
  var backGateBtn = document.getElementById('btn-back-gate'); if (backGateBtn) backGateBtn.addEventListener('click', function() { if (confirm('처음 화면으로 돌아가시겠습니까?\n현재 검색 상태가 초기화됩니다.')) { document.getElementById('app-container').style.display = 'none'; document.getElementById('gate-page').style.display = ''; document.getElementById('gate-step-2').classList.remove('active'); document.getElementById('gate-step-1').classList.add('active'); var selectedCards = document.querySelectorAll('.gate-job-card.selected'); for (var i = 0; i < selectedCards.length; i++) selectedCards[i].classList.remove('selected'); resetAll(false); } });
  ...
  document.addEventListener('keydown', function(e) { if (e.key === '/' && !e.target.matches('input,textarea,select')) { e.preventDefault(); if (searchInput) searchInput.focus(); } if (e.key === 'Escape') { closeSidePanel(); var ho = document.getElementById('help-overlay'); if (ho) ho.classList.remove('active'); var ap = document.getElementById('ai-panel'); if (ap) ap.classList.remove('open'); var sg = document.getElementById('search-suggestions'); if (sg) sg.classList.remove('open'); var cm = document.getElementById('csv-modal-overlay'); if (cm) cm.classList.remove('active'); } if (!e.target.matches('input,textarea,select')) { if (e.key === 'ArrowLeft' && S.page > 1) { S.page--; renderList(); } if (e.key === 'ArrowRight') { var tp = Math.ceil(S.filtered.length/S.rows); if (S.page < tp) { S.page++; renderList(); } } } });
...
function showSearchSuggestions(query) {
  ...
  var ql = query.toLowerCase(); var titleMatches = [], catSet = new Set(), topicSet = new Set(), koEnSuggestions = [];
  for (var i = 0; i < S.courses.length; i++) { var c = S.courses[i]; var searchTitle = (c.title || '') + (c._titleTranslated ? ' ' + c._titleTranslated : ''); if (searchTitle.toLowerCase().indexOf(ql) !== -1) titleMatches.push(c); if (c.category && c.category.toLowerCase().indexOf(ql) !== -1) catSet.add(c.category.split(',')[0].trim()); if (c.topic && c.topic.toLowerCase().indexOf(ql) !== -1) topicSet.add(c.topic.split('|')[0].trim()); }
  titleMatches.sort(function(a,b) { return (b.enrollments||0)-(a.enrollments||0); }); titleMatches = titleMatches.slice(0, 3);
  var catMatches = Array.from(catSet).filter(Boolean).slice(0, 2); var topicMatches = Array.from(topicSet).filter(Boolean).slice(0, 2);
  var entries = Object.entries(KO_EN_MAP); for (var i = 0; i < entries.length; i++) { var ko = entries[i][0], enList = entries[i][1]; var match = ko.indexOf(ql) !== -1; if (!match) { for (var j = 0; j < enList.length; j++) { if (enList[j].indexOf(ql) !== -1) { match = true; break; } } } if (match) koEnSuggestions.push({ ko: ko, en: enList[0] }); }
  var html = '';
  ...
  for (var i = 0; i < items.length; i++) { (function(item) { item.addEventListener('click', function() { if (item.dataset.courseId) { var course = S.courses.find(function(x){return x.id===item.dataset.courseId;}); if (course) openSidePanel(course); } else if (item.dataset.category) { setMSValues('f-category',[item.dataset.category]); applyFilters(); } else if (item.dataset.topic) { document.getElementById('search-input').value = item.dataset.topic; applyFilters(); } else if (item.dataset.keyword) { var cur = document.getElementById('search-input').value.trim(); document.getElementById('search-input').value = cur ? cur+', '+item.dataset.keyword : item.dataset.keyword; applyFilters(); } container.classList.remove('open'); }); })(items[i]); }
}

function parseTempList(text) {
  var log = document.getElementById('temp-log'); if (log) log.innerHTML = '<div class="log-entry log-info">📋 파싱 중...</div>';
  var lines = text.split('\n').filter(function(l) { return l.trim().length > 0; }); if (lines.length < 2) { if (log) log.innerHTML += '<div class="log-entry log-error">❌ 최소 2줄 필요</div>'; return; }
  ...
  var titleIdx=-1, titleTransIdx=-1, catIdx=-1, subCatIdx=-1, descIdx=-1, descTransIdx=-1, headlineIdx=-1, headlineTransIdx=-1, langIdx=-1, idIdx=-1, diffIdx=-1, durationIdx=-1, ratingIdx=-1, enrollIdx=-1, updatedIdx=-1;
  for (var i = 0; i < headers.length; i++) { var h = headers[i]; if (h==='title'||h==='제목'||h==='강의명') titleIdx = i; else if (h==='강의명번역'||h==='title_kr'||h==='titletranslated') titleTransIdx = i; else if (h==='category'||h==='카테고리'||h==='분류') catIdx = i; else if (h==='subcategory'||h==='서브카테고리'||h==='소분류') subCatIdx = i; else if (h==='description'||h==='설명'||h==='상세설명') descIdx = i; else if (h==='설명번역'||h==='description_kr') descTransIdx = i; else if (h==='headline'||h==='소개'||h==='강의소개'||h==='강의개요') headlineIdx = i; else if (h==='강의개요번역'||h==='headline_kr') headlineTransIdx = i; else if (h==='language'||h==='언어') langIdx = i; else if (h==='id'||h==='강의id') idIdx = i; else if (h==='#'||h==='no') { if (idIdx === -1) idIdx = i; } else if (h==='difficulty'||h==='난이도'||h==='level') diffIdx = i; else if (h==='duration'||h==='수강시간'||h==='수강시간(h)'||h==='시간') durationIdx = i; else if (h==='rating'||h==='평점') ratingIdx = i; else if (h==='enrollments'||h==='수강생'||h==='리뷰수') enrollIdx = i; else if (h==='updated'||h==='최근업데이트'||h==='업데이트') updatedIdx = i; }
  if (titleIdx === -1) { if (log) log.innerHTML += '<div class="log-entry log-error">❌ title/강의명 컬럼 없음<br>헤더: ' + headers.join(', ') + '</div>'; return; }
  var courses = [];
  for (var i = 1; i < lines.length; i++) { var cols = []; if (separator === ',') { var line = lines[i]; var inQuote = false; var current = ''; for (var j = 0; j < line.length; j++) { if (line[j] === '"') { inQuote = !inQuote; } else if (line[j] === ',' && !inQuote) { cols.push(current.trim()); current = ''; } else { current += line[j]; } } cols.push(current.trim()); } else { cols = lines[i].split(separator).map(function(c) { return c.trim().replace(/^"|"$/g, ''); }); }
    var title = titleIdx < cols.length ? cols[titleIdx] : ''; if (!title) continue;
    ...
    var durationMin = 0; if (durationIdx !== -1 && durationIdx < cols.length) durationMin = Math.round((parseFloat(cols[durationIdx]) || 0) * 60);
    courses.push({ id: idIdx !== -1 && idIdx < cols.length ? String(cols[idIdx]) : 'temp_' + i, title: title, category: catIdx !== -1 && catIdx < cols.length ? cols[catIdx] : '', topic: (subCatIdx !== -1 && subCatIdx < cols.length ? cols[subCatIdx] : '') + (titleTrans ? ' | ' + titleTrans : ''), description: desc + (descTrans ? ' ' + descTrans : ''), headline: headline + (headlineTrans ? ' ' + headlineTrans : ''), language: langIdx !== -1 && langIdx < cols.length ? cols[langIdx] : 'ja', instructor: '', difficulty: diffIdx !== -1 && diffIdx < cols.length ? (cols[diffIdx] || 'All Levels') : 'All Levels', objectives: '', subtitles: '', lastUpdated: updatedIdx !== -1 && updatedIdx < cols.length ? (cols[updatedIdx] || '') : new Date().toISOString(), url: '', image: '', contentLength: durationMin, rating: ratingIdx !== -1 && ratingIdx < cols.length ? (parseFloat(cols[ratingIdx]) || 0) : 0, enrollments: enrollIdx !== -1 && enrollIdx < cols.length ? (parseInt(String(cols[enrollIdx]).replace(/,/g, '')) || 0) : 0, isNew: false, _titleTranslated: titleTrans, _headlineTranslated: headlineTrans, _descTranslated: descTrans, _search: '', _score: 0 });
  }
  if (courses.length === 0) { if (log) log.innerHTML += '<div class="log-entry log-error">❌ 파싱된 강의 없음</div>'; return; }
  if (!S._originalCourses) S._originalCourses = S.courses.slice(); S.courses = courses;
  var restoreBtn = document.getElementById('btn-temp-restore'); if (restoreBtn) restoreBtn.style.display = '';
  if (log) { log.innerHTML += '<div class="log-entry log-success">✅ ' + courses.length + '개 로드!</div>'; log.innerHTML += '<div class="log-entry log-info">' + (titleIdx !== -1 ? '강의명✅ ' : '') + (titleTransIdx !== -1 ? '강의명번역✅ ' : '') + (catIdx !== -1 ? '카테고리✅ ' : '') + (subCatIdx !== -1 ? '서브카테고리✅ ' : '') + (headlineIdx !== -1 ? '강의개요✅ ' : '') + (headlineTransIdx !== -1 ? '강의개요번역✅ ' : '') + (diffIdx !== -1 ? '난이도✅ ' : '') + (durationIdx !== -1 ? '수강시간✅ ' : '') + (ratingIdx !== -1 ? '평점✅ ' : '') + (enrollIdx !== -1 ? '리뷰수✅ ' : '') + (updatedIdx !== -1 ? '업데이트✅ ' : '') + '</div>'; if (titleTransIdx !== -1) log.innerHTML += '<div class="log-entry log-success">🌐 번역 컬럼 감지! 한국어 검색 가능</div>'; log.innerHTML += '<div class="log-entry log-info">샘플: ' + courses[0].title.substring(0, 50) + '</div>'; if (courses[0]._titleTranslated) log.innerHTML += '<div class="log-entry log-info">번역: ' + courses[0]._titleTranslated.substring(0, 50) + '</div>'; }
