// ═══════════════════════════════════════════════════════════
// app.js — 메인 앱 (AI 모드 삭제 + 검색 버튼 + 게이트 직무 반영)
// ═══════════════════════════════════════════════════════════
...
async function loadAdminStatus() {
  try {
    var r = await fetch(WORKER_URL + '/status', { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } });
    var d = await r.json();
    $('#sync-status-value').textContent = d.isComplete ? '✅ 완료' : d.synced ? '⏳ 진행 중' : '❌ 미완료';
    $('#sync-status-sub').textContent = d.syncedAt ? '마지막: ' + new Date(d.syncedAt).toLocaleString('ko-KR') : '기록 없음';
    $('#courses-count-value').textContent = (d.totalCount || 0).toLocaleString();
    $('#courses-count-sub').textContent = d.isComplete ? '완료' : '진행 중';
    $('#chunks-count-value').textContent = d.totalChunks || 0;
  } catch (e) { $('#sync-status-value').textContent = '❌ 연결 실패'; }
  try {
    var r2 = await fetch(WORKER_URL + '/test-token', { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } });
    var d2 = await r2.json();
    $('#api-status-value').textContent = d2.success ? '✅ 정상' : '❌ 오류';
    $('#api-status-sub').textContent = d2.success ? 'GraphQL OK' : '실패';
  } catch (e) { $('#api-status-value').textContent = '❌'; }
}

async function runSync(isReset) {
  var l = $('#sync-log');
  l.innerHTML = '<div class="log-entry log-info">📡 시작...</div>';
  try {
    var r = await fetch(WORKER_URL + '/sync' + (isReset ? '?reset=true' : ''), { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } });
    var d = await r.json();
    l.innerHTML += '<div class="log-entry ' + (d.success ? 'log-success' : 'log-error') + '">' + (d.success ? '✅' : '❌') + ' ' + (d.message || d.error) + '</div>';
  } catch (e) { l.innerHTML += '<div class="log-entry log-error">❌ ' + e.message + '</div>'; }
  loadAdminStatus();
}

async function runAutoSync() {
  var l = $('#sync-log'), b = $('#btn-sync-auto');
  b.disabled = true; b.textContent = '⏳...';
  l.innerHTML = '<div class="log-entry log-info">🚀 자동 시작...</div>';
  var c = 1, go = true;
  while (go) {
    l.innerHTML += '<div class="log-entry log-info">📡 [' + c + ']...</div>';
    l.scrollTop = l.scrollHeight;
    try {
      var ep = c === 1 ? WORKER_URL + '/sync?reset=true' : WORKER_URL + '/sync';
      var r = await fetch(ep, { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } });
      var d = await r.json();
      if (d.error) { l.innerHTML += '<div class="log-entry log-error">❌ ' + d.error + '</div>'; break; }
      l.innerHTML += '<div class="log-entry log-success">✅ ' + d.message + '</div>';
      if (d.stoppedByTimeout) { await new Promise(function(r) { setTimeout(r, 2000); }); c++; }
      else { l.innerHTML += '<div class="log-entry log-success">🎉 완료! ' + d.totalCount + '개</div>'; go = false; }
    } catch (e) { l.innerHTML += '<div class="log-entry log-error">❌ ' + e.message + '</div>'; break; }
    l.scrollTop = l.scrollHeight;
  }
  b.disabled = false; b.textContent = '🚀 자동 전체';
  loadAdminStatus();
}

function manageCuratorPicksAdmin() {
  var c = localStorage.getItem('curator_picks');
  var ids = c ? JSON.parse(c) : ['8324', '1717020', '2360128'];
  var n = prompt("항해사's PICK 강의 ID (쉼표 구분):", ids.join(','));
  if (n !== null) {
    var i = n.split(',').map(function(x) { return x.trim(); }).filter(Boolean);
    localStorage.setItem('curator_picks', JSON.stringify(i));
    var l = $('#picks-log');
    if (l) l.innerHTML = '<div class="log-entry log-success">⭐ ' + i.length + '개 업데이트</div>';
    toast('⭐ ' + i.length + '개 업데이트');
  }
}
...
function viewCurrentPicks() {
  ...
  var ids = c ? JSON.parse(c) : ['8324', '1717020', '2360128'];
  var l = $('#picks-log');
  if (l) l.innerHTML = '<div class="log-entry log-info">' + ids.map(function(id, i) { return (i + 1) + '. ID: ' + id; }).join('<br>') + '</div>';
}

function manageExcludedCourses() {
  var c = localStorage.getItem('excluded_courses');
  var ids = c ? JSON.parse(c) : [];
  var n = prompt('제외할 강의 ID (쉼표 구분):', ids.join(','));
  if (n !== null) {
    var i = n.split(',').map(function(x) { return x.trim(); }).filter(Boolean);
    localStorage.setItem('excluded_courses', JSON.stringify(i));
    var l = $('#excluded-log');
    if (l) l.innerHTML = '<div class="log-entry log-success">🚫 ' + i.length + '개 제외 설정</div>';
    toast('🚫 ' + i.length + '개 제외');
  }
}

function viewExcludedCourses() {
  var c = localStorage.getItem('excluded_courses');
  var ids = c ? JSON.parse(c) : [];
  var l = $('#excluded-log');
  if (l) l.innerHTML = '<div class="log-entry log-info">🚫 제외: ' + ids.length + '개<br>' + (ids.length > 0 ? ids.join(', ') : '없음') + '</div>';
}

async function verifyData() {
  var l = $('#verify-log');
  l.innerHTML = '<div class="log-entry log-info">🔍 검증...</div>';
  var ch = 0, t = 0, s = 0, k = 0, d = 0, r = 0, e = 0;
  while (true) {
    try {
      var res = await fetch(WORKER_URL + '/get-courses?chunk=' + ch, { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } });
      if (!res.ok) break;
      var data = await res.json();
      if (!data || !Array.isArray(data) || data.length === 0) break;
      t += data.length;
      s += data.filter(function(c) { return c.subtitles && c.subtitles !== '없음' && c.subtitles !== ''; }).length;
      k += data.filter(function(c) { return c.subtitles && c.subtitles.toLowerCase().includes('ko'); }).length;
      d += data.filter(function(c) { return c.contentLength && typeof c.contentLength === 'number' && c.contentLength > 0; }).length;
  ...
  var p = function(n) { return t > 0 ? (n / t * 100).toFixed(1) + '%' : '0%'; };
  l.innerHTML += '<div class="log-entry log-success">📚 총: ' + t.toLocaleString() + ' (' + ch + ' chunks)<br>💬 자막: ' + s.toLocaleString() + ' (' + p(s) + ')<br>🇰🇷 한국어: ' + k.toLocaleString() + ' (' + p(k) + ')<br>⏱️ 시간: ' + d.toLocaleString() + ' (' + p(d) + ')<br>⭐ 평점: ' + r.toLocaleString() + ' (' + p(r) + ')<br>👥 수강: ' + e.toLocaleString() + ' (' + p(e) + ')</div>';
}

async function verifySample() {
  var l = $('#verify-log');
  l.innerHTML = '<div class="log-entry log-info">📋 로드...</div>';
  try {
    var r = await fetch(WORKER_URL + '/get-courses?chunk=0', { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } });
    var d = await r.json();
    if (!d || d.length === 0) { l.innerHTML += '<div class="log-entry log-error">❌ 없음</div>'; return; }
    l.innerHTML += '<div class="log-entry"><strong>필드:</strong> ' + Object.keys(d[0]).join(', ') + '</div>';
  } catch (e) { l.innerHTML += '<div class="log-entry log-error">❌ ' + e.message + '</div>'; }
}

async function testGraphQL() {
  var l = $('#api-log');
  l.innerHTML = '<div class="log-entry log-info">🔐 테스트...</div>';
  try {
    var r = await fetch(WORKER_URL + '/test-token', { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } });
    var d = await r.json();
    l.innerHTML += '<div class="log-entry ' + (d.success ? 'log-success' : 'log-error') + '">' + (d.success ? '✅ 성공' : '❌ ' + d.error) + '</div>';
  } catch (e) { l.innerHTML += '<div class="log-entry log-error">❌ ' + e.message + '</div>'; }
}
...
async function testGemini() {
  ...
  l.innerHTML = '<div class="log-entry log-info">🤖 테스트...</div>';
  try {
    var r = await fetch('/api/ai-expand', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: 'python' }) });
    var d = await r.json();
    l.innerHTML += '<div class="log-entry ' + (d.success ? 'log-success' : 'log-error') + '">' + (d.success ? '✅ 성공' : '❌ ' + (d.error || '실패')) + '</div>';
  } catch (e) { l.innerHTML += '<div class="log-entry log-error">❌ ' + e.message + '</div>'; }
}

async function viewChunk() {
  var l = $('#raw-log'), n = ($('#chunk-number').value || '0');
  l.innerHTML = '<div class="log-entry log-info">📋 Chunk ' + n + '...</div>';
  try {
    var r = await fetch(WORKER_URL + '/get-courses?chunk=' + n, { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } });
    if (!r.ok) { l.innerHTML += '<div class="log-entry log-error">❌ 없음</div>'; return; }
    var d = await r.json();
    l.innerHTML += '<div class="log-entry log-success"><strong>📦 ' + d.length + '개</strong><br>' + d.slice(0, 3).map(function(c, i) { return (i + 1) + '. ' + ((c.title || '').substring(0, 40)); }).join('<br>') + '</div>';
  } catch (e) { l.innerHTML += '<div class="log-entry log-error">❌ ' + e.message + '</div>'; }
}
...
async function launch() {
  ...
  try {
    ...
    var allCourses = [];
    if (totalChunks > 0) {
      var m = $('#launch-message');
      if (m) m.textContent = '병렬 로딩 중... (' + totalChunks + ' chunks)';
      var promises = [];
      for (var i = 0; i < totalChunks; i++) {
        (function(idx) {
          promises.push(
            fetch(WORKER_URL + '/get-courses?chunk=' + idx, { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } })
      ...
      while (true) {
        try {
          var r2 = await fetch(WORKER_URL + '/get-courses?chunk=' + ci, { headers: { 'Authorization': 'Bearer ' + WORKER_SECRET } });
          if (!r2.ok) break;
          var d2 = await r2.json();
          if (!d2 || !Array.isArray(d2) || d2.length === 0) break;
          allCourses = allCourses.concat(d2);
          ci++;
          if (ci >= 50) break;
        } catch (e) { break; }
      }
...
function initApp() {
  // 환영 메시지
  var jobEmojis = S.selectedFamilies.map(function(f) { return CURATION[f] ? CURATION[f].emoji : ''; }).join('');
  ...
  if (searchInput) {
    searchInput.addEventListener('input', function() {
      ...
      suggestionTimer = setTimeout(function() { showSearchSuggestions(searchInput.value.trim()); }, 250);
    });
  ...
  // AI 내비
  var aiScanBtn = $('#btn-ai-scan');
  if (aiScanBtn) aiScanBtn.addEventListener('click', handleAIScan);
  var applyAiBtn = $('#btn-apply-ai');
  if (applyAiBtn) applyAiBtn.addEventListener('click', applyAIKeywords);
  var aiSelectAll = $('#btn-ai-select-all');
  ...
  var aiClose = $('#btn-ai-close');
  if (aiClose) aiClose.addEventListener('click', function() { $('#ai-panel').classList.remove('open'); });
  ...
  var rowsSelect = $('#rows-select');
  if (rowsSelect) rowsSelect.addEventListener('change', function() { S.rows = parseInt(rowsSelect.value); S.page = 1; renderList(); });
  ...
  if (checkAll) {
    checkAll.addEventListener('change', function() {
      ...
      if (checkAll.checked) ids.forEach(function(id) { S.selectedIds.add(id); });
  ...
  // CSV 모달 닫기
  var csvModalClose = $('#csv-modal-close');
  if (csvModalClose) csvModalClose.addEventListener('click', function() { $('#csv-modal-overlay').classList.remove('active'); });
  var csvModalOverlay = $('#csv-modal-overlay');
  ...
  document.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Escape') {
      closeSidePanel();
      var ho = $('#help-overlay'); if (ho) ho.classList.remove('active');
      var ap = $('#ai-panel'); if (ap) ap.classList.remove('open');
      var sg = $('#search-suggestions'); if (sg) sg.classList.remove('open');
      var cm = $('#csv-modal-overlay'); if (cm) cm.classList.remove('active');
    }
    if (!e.target.matches('input,textarea,select')) {
      if (e.key === 'ArrowLeft' && S.page > 1) { S.page--; renderList(); }
      if (e.key === 'ArrowRight') {
        var tp = Math.ceil(S.filtered.length / S.rows);
        if (S.page < tp) { S.page++; renderList(); }
      }
    }
  });
}
...
function showSearchSuggestions(query) {
  ...
  // 인기 강의
  var titleMatches = S.courses
    .filter(function(c) { return c.title && c.title.toLowerCase().includes(ql); })
  ...
  S.courses.forEach(function(c) {
    if (c.category && c.category.toLowerCase().includes(ql)) {
      catSet.add(c.category.split(',')[0].trim());
    }
  });
  var catMatches = [...catSet].filter(Boolean).slice(0, 2);

  // 주제
  var topicSet = new Set();
  S.courses.forEach(function(c) {
    if (c.topic && c.topic.toLowerCase().includes(ql)) topicSet.add(c.topic);
  });
  var topicMatches = [...topicSet].filter(Boolean).slice(0, 2);

  // 한영 매핑
  var koEnSuggestions = [];
  Object.entries(KO_EN_MAP).forEach(function(entry) {
    var ko = entry[0], enList = entry[1];
    if (ko.includes(ql) || enList.some(function(en) { return en.includes(ql); })) {
      koEnSuggestions.push({ ko: ko, en: enList[0] });
    }
  });
  ...
  if (titleMatches.length > 0) {
    ...
    titleMatches.forEach(function(c) {
      html += '<div class="suggestion-item" data-course-id="' + c.id + '">' +
        '<span class="suggestion-icon">📖</span>' +
        '<span class="suggestion-text">' + (c.title.length > 50 ? c.title.substring(0, 50) + '...' : c.title) + '</span>' +
        '<span class="suggestion-badge">⭐' + (c.rating ? c.rating.toFixed(1) : '-') + '</span></div>';
    });
  }
  ...
  if (catMatches.length > 0) {
    ...
    catMatches.forEach(function(cat) {
      html += '<div class="suggestion-item" data-category="' + cat + '">' +
        '<span class="suggestion-icon">' + getCatEmoji(cat) + '</span>' +
        '<span class="suggestion-text">' + cat + '</span>' +
        '<span class="suggestion-badge">카테고리 필터</span></div>';
    });
  }
  ...
  if (topicMatches.length > 0) {
    ...
    topicMatches.forEach(function(topic) {
      html += '<div class="suggestion-item" data-topic="' + topic + '">' +
        '<span class="suggestion-icon">🏷️</span>' +
        '<span class="suggestion-text">' + topic + '</span></div>';
    });
  }

  if (koEnSuggestions.length > 0) {
    html += '<div class="suggestion-category">🌐 번역 제안</div>';
    koEnSuggestions.slice(0, 2).forEach(function(s) {
      html += '<div class="suggestion-item" data-keyword="' + s.en + '">' +
        '<span class="suggestion-icon">🔄</span>' +
        '<span class="suggestion-text">' + s.ko + ' → ' + s.en + '</span></div>';
    });
  }
  ...
  container.querySelectorAll('.suggestion-item').forEach(function(item) {
    item.addEventListener('click', function() {
      if (item.dataset.courseId) {
        ...
        var cur = $('#search-input').value.trim();
        $('#search-input').value = cur ? cur + ', ' + item.dataset.keyword : item.dataset.keyword;
        applyFilters();
      }
