...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var gateRocket = document.querySelector('.gate-rocket');
  if (gateRocket) {
    gateRocket.style.cursor = 'pointer';
    gateRocket.addEventListener('click', function() {
      gateClickCount++;
...
function enterAdminMode() {
  ...
  ap.innerHTML = '<div class="admin-container">' +
    ...
    '<div class="admin-sections">' +
      ...
      '<div class="admin-section"><h3>📋 임시 목록 모드</h3><p class="admin-desc">외부 강의 목록(CSV/텍스트)을 업로드하여 임시로 큐레이션합니다. 번역 컬럼(강의명번역, 강의개요번역)이 있으면 한국어 검색이 가능합니다.</p><div class="admin-btn-group"><label class="admin-btn admin-btn-primary" style="cursor:pointer;">📥 CSV 업로드<input type="file" id="temp-csv-upload" accept=".csv,.tsv,.txt" style="display:none;" /></label><button class="admin-btn admin-btn-primary" id="btn-temp-paste">📝 텍스트 붙여넣기</button><button class="admin-btn admin-btn-warning" id="btn-temp-restore" style="display:none;">↩️ 원래 목록 복원</button></div><div id="temp-paste-area" style="display:none;margin-top:0.8rem;"><textarea id="temp-paste-text" rows="8" placeholder="탭 또는 쉼표로 구분된 데이터를 붙여넣으세요.&#10;첫 줄은 헤더 (예: 강의명, 카테고리, 강의개요)&#10;번역 컬럼: 강의명번역, 강의개요번역" style="width:100%;padding:0.8rem;background:var(--bg-card-solid);border:1px solid var(--border);border-radius:var(--radius-xs);color:var(--text-bright);font-size:0.82rem;font-family:Consolas,monospace;resize:vertical;"></textarea><button class="admin-btn admin-btn-primary" id="btn-temp-apply-paste" style="margin-top:0.5rem;">🚀 이 목록으로 전환</button></div><div class="admin-log" id="temp-log"></div></div>' +
...
// ★ 관리자 모드 나가기 — 임시 목록이면 메인으로, 아니면 게이트로
...
function initApp() {
  ...
  if (searchBtn) searchBtn.addEventListener('click', function() { var sg = document.getElementById('search-suggestions'); if (sg) sg.classList.remove('open'); clearTimeout(debounceTimer); S.showAllResults = false; applyFilters(); });

  var filterToggle = document.getElementById('btn-filter-toggle');
  if (filterToggle) filterToggle.addEventListener('click', function() { document.getElementById('filters-grid').classList.toggle('open'); filterToggle.classList.toggle('active'); });
  ...
  if (S.selectedFamilies && S.selectedFamilies.length > 0 && !document.getElementById('search-input').value.trim()) {
    ...
    for (var fi = 0; fi < S.selectedFamilies.length; fi++) {
      ...
      if (famData) { var nameWords = famData.name.split(/[\/·,\s]+/); for (var ni = 0; ni < nameWords.length; ni++) { var w = nameWords[ni].trim(); if (w.length >= 2 && jobKeywords.indexOf(w) === -1) jobKeywords.push(w); } }
    }
...
function showSearchSuggestions(query) {
  ...
  var html = '';
  if (titleMatches.length > 0) { html += '<div class="suggestion-category">🔥 인기 강의</div>'; for (var i = 0; i < titleMatches.length; i++) { var c = titleMatches[i]; var displayTitle = c._titleTranslated ? c._titleTranslated : c.title; html += '<div class="suggestion-item" data-course-id="'+c.id+'"><span class="suggestion-icon">📖</span><span class="suggestion-text">'+(displayTitle.length>50?displayTitle.substring(0,50)+'...':displayTitle)+'</span><span class="suggestion-badge">⭐'+(c.rating?c.rating.toFixed(1):'-')+'</span></div>'; } }
...
function parseTempList(text) {
  ...
  for (var i = 0; i < headers.length; i++) {
    ...
    else if (h==='enrollments'||h==='수강생'||h==='리뷰수'||h==='리뷰수') enrollIdx = i;
