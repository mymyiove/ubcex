(function() {
  ...
  function loadLogs() {
    ...
    for (var i = 0; i < Math.min(logs.length, 100); i++) {
      var log = logs[i]; var time = new Date(log.time);
      var timeStr = time.toLocaleDateString('ko-KR') + ' ' + time.toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' });
      html += '<div class="log-item"><span class="log-time">' + timeStr + '</span><span class="log-user">' + escHtml(log.user) + '</span><span class="log-action">' + escHtml(log.action) + (log.detail ? ' — ' + escHtml(log.detail) : '') + '</span></div>';
    }
    el.innerHTML = html;
  }
  ...
  // === Init ===
  function initAdmin() { initEditors(); initLayoutSelectors(); initFetchButtons(); initImageTemplates(); loadLetterList(); }

  function initImageTemplates() {
    var fields = [['f-cover-image', DEFAULT_IMAGES.cover], ['f-insight-image', DEFAULT_IMAGES.insight], ['f-new-image', DEFAULT_IMAGES.newContent], ['f-curation-image', DEFAULT_IMAGES.curation], ['f-closing-image', DEFAULT_IMAGES.closing]];
    for (var i = 0; i < fields.length; i++) { var input = $('#' + fields[i][0]); if (input && !input.value) input.value = fields[i][1]; }
  }

  function createQuill(sel, ph) {
    return new Quill(sel, { theme: 'snow', modules: { toolbar: [[{ header: [2, 3, 4, false] }], ['bold', 'italic', 'underline', 'strike'], [{ color: [] }, { background: [] }], [{ list: 'ordered' }, { list: 'bullet' }], ['link', 'image'], ['clean']] }, placeholder: ph || '' });
  }
  ...
  function setupPageTabs(section) {
    var tabsEl = $('#' + section + '-tabs'); var editorsEl = $('#' + section + '-editors'); var addBtn = $('#' + section + '-add-page');
    addBtn.addEventListener('click', function() {
      var idx = section === 'insight' ? insightPageCount : promoPageCount;
      var tab = document.createElement('button'); tab.className = 'page-tab'; tab.setAttribute('data-page', idx);
      tab.innerHTML = '페이지 ' + (idx + 1) + '<span class="page-tab-delete" data-page="' + idx + '"> ✕</span>';
      tabsEl.insertBefore(tab, addBtn);
      var wrap = document.createElement('div'); wrap.className = 'editor-wrap'; wrap.setAttribute('data-page', idx); wrap.style.display = 'none';
      wrap.innerHTML = '<div id="' + section + '-editor-' + idx + '"></div>'; editorsEl.appendChild(wrap);
    ...
    tabsEl.addEventListener('click', function(e) {
      if (e.target.classList.contains('page-tab-delete')) {
        e.stopPropagation(); var pg = e.target.getAttribute('data-page');
        var tab = tabsEl.querySelector('.page-tab[data-page="' + pg + '"]');
        var wrap = editorsEl.querySelector('.editor-wrap[data-page="' + pg + '"]');
        if (tab) tab.remove(); if (wrap) wrap.remove(); delete editors[section + '-' + pg];
        var ft = tabsEl.querySelector('.page-tab'); if (ft) switchPage(section, ft.getAttribute('data-page'));
        return;
      }
  ...
  function initLayoutSelectors() {
    ...
    for (var si = 0; si < secs.length; si++) {
      (function(sec) {
        var sel = $('#' + sec + '-layout-selector'); if (!sel) return;
  ...
  function fetchCourses(sec) {
    ...
    if (uncached.length === 0) {
      statusEl.textContent = '✅ ' + ids.length + '개 로드 (캐시)'; statusEl.className = 'fetch-status success';
  ...
  function loadCoursesFromWorker() {
    ...
    return fetch('/api/courses-proxy?action=status').then(function(r) { if (!r.ok) throw new Error('status failed'); return r.json(); })
      .then(function(s) {
        var tc = s.totalChunks || 0; if (tc === 0) throw new Error('no chunks');
        var BATCH = 3; var all = []; var batchIndex = 0;
        function loadBatch() {
          if (batchIndex >= tc) { allCoursesCache = all; return Promise.resolve(all); }
          var end = Math.min(batchIndex + BATCH, tc); var ps = [];
          for (var i = batchIndex; i < end; i++) ps.push(fetch('/api/courses-proxy?chunk=' + i).then(function(r) { return r.ok ? r.json() : []; }).catch(function() { return []; }));
          return Promise.all(ps).then(function(rs) {
            ...
            for (var si = 0; si < statusEls.length; si++) { var el = document.getElementById(statusEls[si]); if (el && !el.textContent.match(/✅|❌/)) el.textContent = '⏳ 강의 데이터 로딩 중... ' + pct + '% (' + all.length + '개)'; }
  ...
  function renderCoursePreview(sec) {
    var container = $('#' + sec + '-course-preview'); var ids = sectionCourses[sec].ids; var layout = sectionCourses[sec].layout;
    if (ids.length === 0) { container.innerHTML = ''; return; }
    var html = '';
    for (var i = 0; i < ids.length; i++) {
      var id = ids[i]; var c = courseCache[id]; var comment = sectionCourses[sec].comments[id] || '';
      ...
      var title = c.title || ''; var thumb = c.image || ''; var metaParts = [];
  ...
  function renderBadgeSelector(id, cur, sec) {
    ...
    for (var i = 0; i < BADGE_OPTIONS.length; i++) {
      var b = BADGE_OPTIONS[i]; var sel = cur === b.id ? ' selected' : ''; var isCustom = b.id.indexOf('custom_') === 0;
  ...
  function bindPreviewEvents(sec) {
    ...
    for (var i = 0; i < removeBtns.length; i++) {
      removeBtns[i].addEventListener('click', function() {
        var s = this.getAttribute('data-section'); var id = this.getAttribute('data-id');
        ...
        $('#' + iid).value = sectionCourses[s].ids.join(', '); renderCoursePreview(s);
      });
    }
    var badgeBtns = $$('#' + sec + '-course-preview .badge-opt');
    for (var i = 0; i < badgeBtns.length; i++) {
      badgeBtns[i].addEventListener('click', function() {
        var s = this.getAttribute('data-section'); var id = this.getAttribute('data-id'); var b = this.getAttribute('data-badge');
    ...
    for (var i = 0; i < addBBtns.length; i++) {
      addBBtns[i].addEventListener('click', function() {
        var s = this.getAttribute('data-section'); var id = this.getAttribute('data-id');
        var inp = this.parentElement.querySelector('.custom-badge-input'); var val = inp.value.trim(); if (!val) return;
        var nid = 'custom_' + Date.now(); BADGE_OPTIONS.push({ id: nid, ko: val }); customBadges.push({ id: nid, ko: val });
    ...
    for (var i = 0; i < badgeDelBtns.length; i++) {
      badgeDelBtns[i].addEventListener('click', function(e) {
        e.stopPropagation(); var badgeId = this.getAttribute('data-badge-id');
    ...
    for (var i = 0; i < cInputs.length; i++) {
      cInputs[i].addEventListener('input', function() { sectionCourses[this.getAttribute('data-section')].comments[this.getAttribute('data-id')] = this.value; });
    }
    ...
    for (var i = 0; i < aiSingleBtns.length; i++) {
      aiSingleBtns[i].addEventListener('click', function() {
        var s = this.getAttribute('data-section'); var id = this.getAttribute('data-id'); var c = courseCache[id];
        if (!c) { toast('강의 데이터 없음', 'error'); return; }
        var btn = this; btn.disabled = true; btn.textContent = '⏳';
        apiCall('/letter-ai-comment', 'POST', { courses: [{ id: id, title: c.title || '', rating: c.rating ? String(c.rating) : '', duration: c.contentLength ? formatDuration(c.contentLength) : '', instructor: c.instructor || '', category: c.category || '', difficulty: c.difficulty || '', headline: c.headline || '', description: c.description || '', objectives: c.objectives || '' }] })
        .then(function(res) {
          btn.disabled = false; btn.textContent = '🤖';
          if (res.success && res.comments && res.comments.length > 0) {
            var commentText = res.comments[0].comment_ko || '';
            if (commentText) { sectionCourses[s].comments[id] = commentText; var ta = document.querySelector('#' + s + '-course-preview .course-comment-input[data-id="' + id + '"]'); if (ta) ta.value = commentText; toast('🤖 AI 코멘트 생성!', 'success'); }
  ...
  function loadLetterList() {
    apiCall('/letter-list').then(function(res) {
      ...
      for (var i = 0; i < res.data.length; i++) {
        var item = res.data[i]; var sc = 'status-' + item.status;
        ...
        html += '<div class="letter-card-meta"><span class="status-badge ' + sc + '">' + st + '</span><span>' + (item.updatedAt ? new Date(item.updatedAt).toLocaleDateString('ko-KR') : '') + '</span></div>';
      ...
      for (var i = 0; i < actionBtns.length; i++) {
        actionBtns[i].addEventListener('click', function(e) {
          e.stopPropagation();
          var m = this.getAttribute('data-month'); var action = this.getAttribute('data-action');
          if (action === 'edit') loadLetter(m);
          if (action === 'email') {
            var sub = prompt('[' + m + '호] Subdomain을 입력하세요.', 'woongjin'); if (sub === null) return;
            var eLang = prompt('언어 선택 (ko/en)', 'ko'); if (eLang === null) return;
            eLang = eLang.trim().toLowerCase(); if (eLang !== 'en') eLang = 'ko';
      ...
      for (var i = 0; i < cards.length; i++) {
        cards[i].addEventListener('click', function(e) { if (e.target.closest('.letter-card-action')) return; loadLetter(this.getAttribute('data-month')); });
      }
    });
  }
  ...
  $('#btn-new-letter').addEventListener('click', function() {
    ...
    $('#editor-title').textContent = '✏️ 새 레터'; $('#btn-delete').style.display = 'none'; $('#btn-copy-email').style.display = 'none'; $('#last-saved-info').textContent = '';
    switchPanel('editor');
  });
  ...
  function loadLetter(month) {
    ...
    apiCall('/letter-get?month=' + month).then(function(res) {
      hideLoading(); if (!res.success) { toast('불러오기 실패', 'error'); return; }
      var d = res.data; currentMonth = d.month;
      $('#f-month').value = d.month; $('#f-month').disabled = true; $('#f-status').value = d.status || 'draft';
      ...
      if (d.curation && d.curation.tags) { var tt = []; for (var i = 0; i < d.curation.tags.length; i++) tt.push(d.curation.tags[i].ko || d.curation.tags[i]); $('#f-curation-tags').value = tt.join(', '); }
      if (d.insight && d.insight.courseIds && d.insight.courseIds.length > 0) { $('#f-insight-course-ids').value = d.insight.courseIds.join(', '); sectionCourses.insight.ids = d.insight.courseIds; sectionCourses.insight.comments = d.insight.courseComments || {}; sectionCourses.insight.badges = d.insight.courseBadges || {}; if (d.insight.layout) sectionCourses.insight.layout = d.insight.layout; }
      if (d.newContent && d.newContent.courseIds && d.newContent.courseIds.length > 0) { $('#f-new-course-ids').value = d.newContent.courseIds.join(', '); sectionCourses.new.ids = d.newContent.courseIds; sectionCourses.new.comments = d.newContent.courseComments || {}; sectionCourses.new.badges = d.newContent.courseBadges || {}; if (d.newContent.layout) sectionCourses.new.layout = d.newContent.layout; }
      if (d.curation && d.curation.courseIds && d.curation.courseIds.length > 0) { $('#f-curation-ids').value = d.curation.courseIds.join(', '); sectionCourses.curation.ids = d.curation.courseIds; sectionCourses.curation.comments = d.curation.courseComments || {}; sectionCourses.curation.badges = d.curation.courseBadges || {}; if (d.curation.layout) sectionCourses.curation.layout = d.curation.layout; }
      ...
      $('#editor-title').textContent = '✏️ ' + d.month + '호 편집'; $('#btn-delete').style.display = ''; $('#btn-copy-email').style.display = '';
      if (d.updatedAt) { var info = new Date(d.updatedAt).toLocaleString('ko-KR'); if (d.lastEditor) info += ' by ' + d.lastEditor; $('#last-saved-info').textContent = '💾 ' + info; }
      var sectionsToFetch = [];
      ...
      switchPanel('editor'); addLog('레터 열기', d.month + '호');
      if (sectionsToFetch.length > 0) {
        toast(d.month + '호 로드 중... 강의 데이터 불러오는 중');
        loadCoursesFromWorker().then(function(all) {
          for (var si = 0; si < sectionsToFetch.length; si++) { var sec = sectionsToFetch[si]; var ids = sectionCourses[sec].ids;
            for (var i = 0; i < ids.length; i++) { if (!courseCache[ids[i]]) { for (var j = 0; j < all.length; j++) { if (String(all[j].id) === String(ids[i])) { courseCache[ids[i]] = all[j]; break; } } } }
            renderCoursePreview(sec);
          }
  ...
  $('#btn-save').addEventListener('click', function() {
    var month = $('#f-month').value.trim(); if (!month || !/^\d{4}-\d{2}$/.test(month)) { toast('YYYY-MM 형식으로!', 'error'); return; }
    var insightPages = []; var iw = $$('#insight-editors .editor-wrap');
    ...
    var promoPages = []; var pw = $$('#promo-editors .editor-wrap');
    ...
    var tagsRaw = $('#f-curation-tags').value.trim(); var tags = [];
    if (tagsRaw) { var parts = tagsRaw.split(','); for (var i = 0; i < parts.length; i++) { var t = parts[i].trim(); if (t) tags.push({ ko: t, en: '' }); } }
    var body = { month: month, title_ko: $('#f-title-ko').value.trim(), subtitle_ko: $('#f-subtitle-ko').value.trim(), coverImage: $('#f-cover-image').value.trim(), status: $('#f-status').value, lastEditor: currentUser, insight_image: $('#f-insight-image').value.trim(), insight_pages: insightPages, insight_courseIds: sectionCourses.insight.ids, insight_courseComments: sectionCourses.insight.comments, insight_courseBadges: sectionCourses.insight.badges, insight_layout: sectionCourses.insight.layout, newContent_image: $('#f-new-image').value.trim(), newContent_editorHtml_ko: editors['new'] ? editors['new'].root.innerHTML : '', newContent_summary_ko: $('#f-new-summary-ko').value.trim(), newContent_courseIds: sectionCourses.new.ids, newContent_courseComments: sectionCourses.new.comments, newContent_courseBadges: sectionCourses.new.badges, newContent_layout: sectionCourses.new.layout, curation_image: $('#f-curation-image').value.trim(), curation_intro_ko: $('#f-curation-intro-ko').value.trim(), curation_tags: tags, curation_courseIds: sectionCourses.curation.ids, curation_courseComments: sectionCourses.curation.comments, curation_courseBadges: sectionCourses.curation.badges, curation_layout: sectionCourses.curation.layout, closing_image: $('#f-closing-image').value.trim(), closing_ko: editors['closing'] ? editors['closing'].root.innerHTML : '', promo_pages: promoPages };
    showLoading();
    apiCall('/letter-save', 'POST', body).then(function(res) {
      hideLoading();
      if (res.success) { currentMonth = month; $('#f-month').disabled = true; $('#btn-delete').style.display = ''; $('#btn-copy-email').style.display = ''; $('#last-saved-info').textContent = '💾 ' + new Date().toLocaleString('ko-KR') + ' by ' + currentUser; addLog('레터 저장', month + '호'); toast(res.message, 'success'); }
  ...
  $('#btn-delete').addEventListener('click', function() {
    if (!currentMonth) return; if (!confirm(currentMonth + '호를 삭제하시겠습니까?')) return;
    showLoading();
  ...
  $('#btn-copy-email').addEventListener('click', function() {
    ...
    var sub = prompt('[' + currentMonth + '호] Subdomain을 입력하세요.', 'woongjin'); if (sub === null) return;
    var emailLang = prompt('언어 선택 (ko/en)', 'ko'); if (emailLang === null) return;
    emailLang = emailLang.trim().toLowerCase(); if (emailLang !== 'en') emailLang = 'ko';
    generateEmail(currentMonth, sub.trim(), emailLang);
  });

  // === Helpers ===
  function switchPanel(name) {
    var panels = $$('.admin-panel'); for (var i = 0; i < panels.length; i++) panels[i].classList.remove('active');
    $('#panel-' + name).classList.add('active');
    var nb = $$('.admin-nav-btn[data-panel]'); for (var i = 0; i < nb.length; i++) nb[i].classList.toggle('active', nb[i].getAttribute('data-panel') === name);
  }

  function clearEditor() {
    $('#f-month').value = ''; $('#f-month').disabled = false; $('#f-status').value = 'draft'; $('#f-title-ko').value = ''; $('#f-subtitle-ko').value = '';
    $('#f-cover-image').value = DEFAULT_IMAGES.cover; $('#f-insight-image').value = DEFAULT_IMAGES.insight; $('#f-new-image').value = DEFAULT_IMAGES.newContent; $('#f-new-summary-ko').value = '';
    $('#f-curation-image').value = DEFAULT_IMAGES.curation; $('#f-curation-intro-ko').value = ''; $('#f-curation-tags').value = ''; $('#f-curation-ids').value = ''; $('#f-closing-image').value = DEFAULT_IMAGES.closing;
    $('#btn-copy-email').style.display = 'none';
    if ($('#f-insight-course-ids')) $('#f-insight-course-ids').value = ''; if ($('#f-new-course-ids')) $('#f-new-course-ids').value = '';
    sectionCourses.insight = { ids: [], comments: {}, badges: {}, layout: 'card' }; sectionCourses.new = { ids: [], comments: {}, badges: {}, layout: 'highlight' }; sectionCourses.curation = { ids: [], comments: {}, badges: {}, layout: 'list' };
  ...
  function generateEmail(month, sub, emailLang) {
    ...
    apiCall('/letter-get?month=' + month).then(function(res) {
      ...
      if (allIds.length === 0) { doEmailCopy(letter, sub, {}, month, emailLang); return; }
      var cMap = {}; var missingIds = [];
      for (var i = 0; i < allIds.length; i++) { var id = String(allIds[i]); if (courseCache[id]) cMap[id] = courseCache[id]; else if (courseCache[allIds[i]]) cMap[id] = courseCache[allIds[i]]; else missingIds.push(id); }
      if (missingIds.length === 0) { doEmailCopy(letter, sub, cMap, month, emailLang); return; }
      fetch('/api/courses-proxy?ids=' + encodeURIComponent(missingIds.join(','))).then(function(r) { return r.ok ? r.json() : []; }).then(function(courses) {
        if (Array.isArray(courses)) { for (var i = 0; i < courses.length; i++) { var cid = String(courses[i].id); cMap[cid] = courses[i]; courseCache[cid] = courses[i]; } }
        doEmailCopy(letter, sub, cMap, month, emailLang);
      }).catch(function() { doEmailCopy(letter, sub, cMap, month, emailLang); });
    }).catch(function(e) { hideLoading(); toast('실패: ' + e.message, 'error'); });
  }

  function doEmailCopy(letter, sub, cMap, month, emailLang) {
    var html = buildEmailHtml(letter, sub, cMap, emailLang); hideLoading();
    var ta = document.createElement('textarea'); ta.value = html; ta.style.cssText = 'position:fixed;opacity:0;left:-9999px;';
    document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta);
    var totalIds = 0; var foundIds = Object.keys(cMap).length;
    ...
    toast(msg, 'success'); addLog('이메일 HTML 복사', month + '호 / ' + langLabel + ' / sub=' + sub);
  }

  // 이메일 헬퍼
  function emailAbsUrl(url) { if (!url) return ''; if (url.indexOf('http') === 0) return url; return EMAIL_IMAGE_BASE + url.replace(/^\//, ''); }
  function emailCourseUrl(slug, sub) { var base = sub ? 'https://' + sub + '.udemy.com' : 'https://www.udemy.com'; if (!slug) return base; if (slug.indexOf('http') === 0) return slug.replace('www.udemy.com', sub ? sub + '.udemy.com' : 'www.udemy.com'); return base + '/course/' + slug + '/'; }
  function emailCleanHtml(html) {
    if (!html) return ''; var F = EMAIL_FONT;
    html = html.replace(/<p><br><\/p>/g, '<br>'); html = html.replace(/ class="ql-[^"]*"/g, '');
    ...
    html = html.replace(/<ul>/g, '<ul style="margin:0 0 12px;padding-left:20px;">'); html = html.replace(/<ol>/g, '<ol style="margin:0 0 12px;padding-left:20px;">');
  ...
  function emailBadge(bid) {
    ...
    var b = map[bid]; if (!b) { var customs = JSON.parse(localStorage.getItem('letter_custom_badges') || '[]'); for (var i = 0; i < customs.length; i++) { if (customs[i].id === bid) { b = { t: customs[i].ko, bg: '#ede9fe', c: '#5b21b6' }; break; } } }
    if (!b) return ''; return '<span style="display:inline-block;padding:3px 10px;background:' + b.bg + ';color:' + b.c + ';border-radius:8px;font-size:11px;font-weight:700;margin-bottom:6px;">' + b.t + '</span>';
  }
  ...
  function emailDifficulty(d, isEn) { if (isEn) return d || 'All Levels'; var m = { 'Beginner': '초급', 'Intermediate': '중급', 'Expert': '고급', 'All Levels': '모든 수준', 'ALL_LEVELS': '모든 수준', 'BEGINNER': '초급', 'INTERMEDIATE': '중급', 'EXPERT': '고급' }; return m[d] || d || ''; }

  function buildEmailHtml(letter, sub, coursesMap, emailLang) {
    var F = EMAIL_FONT; var isEn = emailLang === 'en';
    var campusUrl = sub ? 'https://' + sub + '.udemy.com' : 'https://www.udemy.com';
    var titleKo = (letter.title && letter.title.ko) ? letter.title.ko : ''; var titleEn = (letter.title && letter.title.en) ? letter.title.en : titleKo; var title = isEn ? titleEn : titleKo;
    var subtitleKo = (letter.subtitle && letter.subtitle.ko) ? letter.subtitle.ko : ''; var subtitleEn = (letter.subtitle && letter.subtitle.en) ? letter.subtitle.en : subtitleKo; var subtitle = isEn ? subtitleEn : subtitleKo;
    var issueLabel = isEn ? (letter.month + ' Issue') : (letter.month + '호');
    var greeting = isEn ? (sub ? 'Monthly letter for ' + sub.toUpperCase() + ' learners' : 'Monthly letter for learners') : (sub ? sub.toUpperCase() + ' 학습자님을 위한 이달의 레터' : '학습자님을 위한 이달의 레터');
    ...
    var txtUnsub = emailText(isEn, '수신거부', 'Unsubscribe');
    var txtNote = emailText(isEn, '💡 강의명을 클릭하면 해당 강의 페이지로 연결됩니다.', '💡 Click a course title to visit the course page.');
    var txtLiked = emailText(isEn, '이번 달 추천 강의가 마음에 드셨나요?', 'Did you enjoy this month\'s recommended courses?');
    var txtFree = emailText(isEn, '💡 지금 바로 학습장에서 무료로 수강하세요!', '💡 Start learning for free on your Learning Hub!');
    ...
    // 헤더
    h += '<tr><td style="padding:14px 24px;border-bottom:1px solid #f0eeff;font-family:' + F + ';"><table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"><tr><td style="font-family:' + F + ';"><img src="https://www.udemy.com/staticx/udemy/images/v7/logo-udemy.svg" alt="Udemy" height="22" style="vertical-align:middle;"> <span style="color:#7c6cf0;font-size:14px;font-weight:700;vertical-align:middle;margin-left:6px;font-family:' + F + ';">Letter</span></td><td align="right" style="font-size:12px;color:#9090aa;font-family:' + F + ';">' + escHtml(issueLabel) + '</td></tr></table></td></tr>\n';
    ...
    if (secDefs.length > 0) {
      h += '<tr><td style="padding:28px 32px;font-family:' + F + ';" class="m-pad"><p style="margin:0 0 14px;font-size:11px;font-weight:700;color:#7c6cf0;letter-spacing:2px;font-family:' + F + ';">INDEX</p><table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%"><tr>';
      for (var si = 0; si < secDefs.length; si++) { var w = Math.floor(100 / secDefs.length); h += '<td width="' + w + '%" valign="top" style="padding:14px 10px;background:#f5f3ff;border-radius:12px;text-align:center;font-family:' + F + ';" class="m-stack"><p style="margin:0 0 2px;font-size:10px;font-weight:700;color:#7c6cf0;font-family:' + F + ';">' + secDefs[si].n + '</p><p style="margin:0 0 4px;font-size:22px;line-height:1;">' + secDefs[si].i + '</p><p style="margin:0;font-size:13px;font-weight:700;color:#1a1a2e;font-family:' + F + ';">' + escHtml(secDefs[si].t) + '</p></td>'; if (si < secDefs.length - 1) h += '<td width="8" style="font-size:0;">&nbsp;</td>'; }
      h += '</tr></table></td></tr>\n';
    }

    // 헬퍼
    function secHead(tag, icon, title) { return '<tr><td style="padding:28px 32px 12px;font-family:' + F + ';" class="m-pad"><p style="margin:0 0 4px;font-size:11px;font-weight:700;color:#7c6cf0;letter-spacing:2px;font-family:' + F + ';">' + escHtml(tag) + '</p><h2 style="margin:0;font-size:22px;font-weight:800;color:#1a1a2e;font-family:' + F + ';">' + icon + ' ' + escHtml(title) + '</h2></td></tr>\n'; }
    function secImg(url) { if (!url) return ''; return '<tr><td style="padding:0 32px 16px;" class="m-pad"><img src="' + escHtml(emailAbsUrl(url)) + '" alt="" width="536" style="display:block;width:536px;max-width:100%;height:auto;border-radius:12px;" class="m-full"></td></tr>\n'; }
    ...
    function courseCards(sData) {
      ...
      for (var ci = 0; ci < ids.length; ci++) { var c = coursesMap[String(ids[ci])]; if (!c) continue; var curl = emailCourseUrl(c.url || c.slug, sub); var cimg = emailAbsUrl(c.image); var badge = badges[ids[ci]]; var cmt = isEn ? (emailGetCommentEn(comments[ids[ci]]) || emailGetComment(comments[ids[ci]])) : emailGetComment(comments[ids[ci]]);
        ...
        out += '<td valign="top" style="padding:14px 16px;font-family:' + F + ';">'; if (badge) out += emailBadge(badge) + '<br>';
        ...
        var meta = []; if (c.rating) meta.push('⭐ ' + Number(c.rating).toFixed(1)); if (c.contentLength) meta.push('⏱️ ' + formatDuration(c.contentLength)); if (c.instructor) meta.push('👤 ' + escHtml((c.instructor || '').substring(0, 25))); if (c.difficulty) meta.push('📊 ' + emailDifficulty(c.difficulty, isEn));
        ...
        out += '<a href="' + escHtml(curl) + '" target="_blank" style="display:inline-block;margin-top:8px;font-size:12px;color:#7c6cf0;text-decoration:none;font-weight:600;font-family:' + F + ';">' + btnStart + '</a></td></tr></table></td></tr>\n';
      } return out;
    }

    function curList(cData) {
      var ids = cData.courseIds || []; var comments = cData.courseComments || {}; var badges = cData.courseBadges || {}; var out = '';
      for (var ci = 0; ci < ids.length; ci++) { var c = coursesMap[String(ids[ci])]; if (!c) continue; var curl = emailCourseUrl(c.url || c.slug, sub); var badge = badges[ids[ci]]; var cmt = isEn ? (emailGetCommentEn(comments[ids[ci]]) || emailGetComment(comments[ids[ci]])) : emailGetComment(comments[ids[ci]]);
        ...
        out += '<td valign="top" style="padding:14px 0 14px 8px;font-family:' + F + ';">'; if (badge) out += emailBadge(badge) + '<br>';
        ...
        var meta = []; if (c.rating) meta.push('⭐ ' + Number(c.rating).toFixed(1)); if (c.contentLength) meta.push('⏱️ ' + formatDuration(c.contentLength)); if (c.difficulty) meta.push('📊 ' + emailDifficulty(c.difficulty, isEn)); if (c.instructor) meta.push('👤 ' + escHtml((c.instructor || '').substring(0, 25)));
        if (meta.length > 0) out += '<p style="margin:5px 0 0;font-size:11px;color:#9090aa;line-height:1.5;font-family:' + F + ';">' + meta.join(' · ') + '</p>';
        ...
        out += '<a href="' + escHtml(curl) + '" target="_blank" style="display:inline-block;margin-top:8px;padding:6px 16px;background:#7c6cf0;color:#ffffff;text-decoration:none;border-radius:18px;font-size:12px;font-weight:600;font-family:' + F + ';">' + btnStartFull + '</a></td></tr></table></td></tr>\n';
        if (ci === 2 && ids.length > 4) { out += '<tr><td style="padding:12px 32px;font-family:' + F + ';" class="m-pad"><div style="background:#f5f3ff;border:1px solid #f0eeff;border-radius:12px;padding:16px;text-align:center;"><p style="margin:0 0 10px;font-size:13px;color:#4a4a6a;font-family:' + F + ';">' + txtFree + '</p><a href="' + escHtml(campusUrl) + '" target="_blank" style="display:inline-block;padding:10px 24px;background:#7c6cf0;color:#ffffff;text-decoration:none;border-radius:20px;font-size:13px;font-weight:700;font-family:' + F + ';">' + btnCampus + '</a></div></td></tr>\n'; }
      } return out;
    }

    // 인사이트
    if (letter.insight) { h += secHead('CONTENT 1', '📊', emailText(isEn, '트렌드 인사이트', 'Trend Insights')); h += secImg(letter.insight.image);
      if (letter.insight.pages) { for (var pi = 0; pi < letter.insight.pages.length; pi++) { var pk = isEn ? 'html_en' : 'html_ko'; var ph = emailCleanHtml(letter.insight.pages[pi][pk] || letter.insight.pages[pi].html_ko || ''); if (ph.replace(/<[^>]*>/g, '').trim()) h += '<tr><td style="padding:0 32px 16px;font-family:' + F + ';" class="m-pad"><div style="background:#ffffff;border:1px solid #f0eeff;border-radius:12px;padding:24px;border-top:4px solid #7c6cf0;">' + ph + '</div></td></tr>\n'; } }
      if (letter.insight.courseIds && letter.insight.courseIds.length > 0) h += courseCards(letter.insight);
    }
    // 신규
    if (letter.newContent) { h += divider() + secHead('CONTENT 2', '✨', emailText(isEn, '신규 콘텐츠', 'New Content')); h += secImg(letter.newContent.image);
      if (letter.newContent.editorHtml) { var ek = isEn ? 'en' : 'ko'; var nh = emailCleanHtml(letter.newContent.editorHtml[ek] || letter.newContent.editorHtml.ko || ''); if (nh.replace(/<[^>]*>/g, '').trim()) h += '<tr><td style="padding:0 32px 16px;font-family:' + F + ';" class="m-pad"><div style="background:#ffffff;border:1px solid #f0eeff;border-radius:12px;padding:24px;">' + nh + '</div></td></tr>\n'; }
      if (letter.newContent.courseIds && letter.newContent.courseIds.length > 0) h += courseCards(letter.newContent);
      if (letter.newContent.summary) { var st = isEn ? (letter.newContent.summary.en || letter.newContent.summary.ko || '') : (letter.newContent.summary.ko || ''); if (st) h += '<tr><td style="padding:0 32px 16px;font-family:' + F + ';" class="m-pad"><div style="background:#f5f3ff;border-radius:12px;padding:16px;text-align:center;"><p style="margin:0;font-size:14px;color:#4a4a6a;line-height:1.7;font-family:' + F + ';">' + escHtml(st) + '</p></div></td></tr>\n'; }
    }
    // 큐레이션
    if (letter.curation) { h += divider() + secHead('CONTENT 3', '🎯', emailText(isEn, '이달의 큐레이션', 'Monthly Curation')); h += secImg(letter.curation.image);
      if (letter.curation.intro || (letter.curation.tags && letter.curation.tags.length > 0)) { h += '<tr><td style="padding:0 32px 16px;font-family:' + F + ';" class="m-pad"><div style="background:#ffffff;border:1px solid #f0eeff;border-radius:12px;padding:20px;border-left:4px solid #7c6cf0;">';
        if (letter.curation.intro) { var it = isEn ? (letter.curation.intro.en || letter.curation.intro.ko || '') : (letter.curation.intro.ko || ''); if (it) h += '<p style="margin:0 0 12px;font-size:14px;color:#4a4a6a;line-height:1.7;font-family:' + F + ';">' + escHtml(it) + '</p>'; }
        if (letter.curation.tags && letter.curation.tags.length > 0) { h += '<div style="margin-top:8px;">'; for (var ti = 0; ti < letter.curation.tags.length; ti++) { var tag = letter.curation.tags[ti]; var tt = isEn ? (tag.en || tag.ko || (typeof tag === 'string' ? tag : '')) : (tag.ko || (typeof tag === 'string' ? tag : '')); h += '<span style="display:inline-block;padding:5px 14px;background:#7c6cf0;color:#ffffff;border-radius:20px;font-size:12px;font-weight:600;margin:0 6px 6px 0;font-family:' + F + ';">' + escHtml(tt) + '</span>'; } h += '</div>'; }
        h += '</div></td></tr>\n'; }
      if (letter.curation.courseIds && letter.curation.courseIds.length > 0) h += curList(letter.curation);
      h += '<tr><td style="padding:0 32px 8px;text-align:center;font-family:' + F + ';" class="m-pad"><p style="margin:0;font-size:12px;color:#9090aa;background:#f5f3ff;padding:10px;border-radius:8px;font-family:' + F + ';">' + txtNote + '</p></td></tr>\n';
    }
    // 홍보
    if (letter.promo && letter.promo.pages) { var hasP = false; for (var pi2 = 0; pi2 < letter.promo.pages.length; pi2++) { if (letter.promo.pages[pi2].html_ko && letter.promo.pages[pi2].html_ko.replace(/<[^>]*>/g, '').trim()) hasP = true; }
      if (hasP) { h += divider() + secHead('PROMOTION', '📢', emailText(isEn, '홍보', 'Promotion')); for (var pi3 = 0; pi3 < letter.promo.pages.length; pi3++) { var ppk = isEn ? 'html_en' : 'html_ko'; var proH = emailCleanHtml(letter.promo.pages[pi3][ppk] || letter.promo.pages[pi3].html_ko || ''); if (proH.replace(/<[^>]*>/g, '').trim()) h += '<tr><td style="padding:0 32px 16px;font-family:' + F + ';" class="m-pad"><div style="background:#ffffff;border:1px solid #f0eeff;border-radius:12px;padding:24px;">' + proH + '</div></td></tr>\n'; } }
    }
    // CTA
    h += '<tr><td style="padding:24px 32px;font-family:' + F + ';" class="m-pad"><div style="background:#f5f3ff;border:1px solid #f0eeff;border-radius:16px;padding:28px;text-align:center;"><p style="margin:0 0 16px;font-size:15px;color:#4a4a6a;font-family:' + F + ';">' + txtLiked + '</p><a href="' + escHtml(campusUrl) + '" target="_blank" style="display:inline-block;padding:14px 32px;background:#7c6cf0;color:#ffffff;text-decoration:none;border-radius:28px;font-size:15px;font-weight:700;font-family:' + F + ';">' + btnCampus + '</a></div></td></tr>\n';
    // 마무리
    var closingMsg = ''; if (letter.closing && letter.closing.message) closingMsg = isEn ? (letter.closing.message.en || letter.closing.message.ko || '') : (letter.closing.message.ko || '');
    var closingDefault = isEn ? 'Every month, we curate the most relevant content and insights<br>to help you apply new skills directly to your work.<br><br>We look forward to making your learning journey even more enjoyable<br>with Udemy Letter. 🚀' : '매월 변화하는 업무 환경과 학습 트렌드에 맞춰,<br>바로 실무에 적용할 수 있는 콘텐츠와 인사이트를 엄선해 소개드립니다.<br><br>앞으로도 여러분의 성장을 더 즐겁게 만드는<br>Udemy Letter로 함께하겠습니다. 🚀';
    ...
    h += '<p style="margin:0 0 8px;font-size:28px;line-height:1;">📮</p><p style="margin:0 0 4px;font-size:18px;font-weight:800;color:#7c6cf0;font-family:' + F + ';">Udemy Letter</p>';
    ...
    h += '</td></tr>\n';
    // 푸터
    h += '<tr><td style="padding:20px 32px;text-align:center;background:#ffffff;border-top:1px solid #f0eeff;font-family:' + F + ';" class="m-pad"><p style="margin:0 0 8px;font-size:12px;color:#9090aa;font-family:' + F + ';">Udemy Letter | ' + (isEn ? 'Woongjin ThinkBig' : '웅진씽크빅') + ' © 2026</p><p style="margin:0;"><a href="#unsubscribe" style="font-size:11px;color:#9090aa;text-decoration:underline;font-family:' + F + ';">' + txtUnsub + '</a></p></td></tr>\n';
