(function() {
  ...
  // ==========================================
  // === Email HTML Generation v5 (호환성 최적화) ===
  // ==========================================
  ...
  function doEmailCopy(letter, sub, cMap, month, emailLang) {
    ...
    function fallbackCopy() {
      ...
      box.style.cssText = 'background:#ffffff;padding:24px;border-radius:12px;width:90%;max-width:700px;box-shadow:0 10px 25px rgba(0,0,0,0.2);';
      var title = document.createElement('h3');
      title.textContent = '⚠️ 자동 복사 실패 (수동으로 복사해 주세요)';
      title.style.cssText = 'margin:0 0 10px 0; color:#1a1a2e; font-size:18px; font-weight:800;';
      var desc = document.createElement('p');
      desc.textContent = '보안 환경(HTTP)이 아니거나 브라우저 정책에 의해 자동 복사가 차단되었습니다. 아래 박스 안을 클릭한 뒤 전체 복사(Ctrl+C 또는 Cmd+C) 하시면 됩니다.';
      desc.style.cssText = 'margin:0 0 16px 0; font-size:13px; color:#4a4a6a; line-height:1.5;';
      var ta = document.createElement('textarea');
      ta.value = html;
      ta.style.cssText = 'width:100%;height:350px;font-family:monospace;font-size:12px;padding:12px;box-sizing:border-box;border:1px solid #ccc;border-radius:8px;resize:vertical;background:#f9f9f9;';
      var closeWrap = document.createElement('div');
      closeWrap.style.cssText = 'text-align:right; margin-top:16px;';
      var closeBtn = document.createElement('button');
      closeBtn.textContent = '닫기';
      closeBtn.style.cssText = 'padding:10px 24px;background:#7c6cf0;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:700;';
      ...
      ta.select();
      addLog('이메일 HTML 수동 복사', logMsg);
      toast('수동 복사 창이 열렸습니다', 'error');
    }

    if (navigator.clipboard && window.isSecureContext) {
      navigator.clipboard.writeText(html).then(onSuccess).catch(function() { fallbackCopy(); });
    } else {
      try {
        var ta = document.createElement('textarea');
        ta.value = html;
  ...
  function buildEmailHtml(letter, sub, coursesMap, emailLang) {
    ...
    h += '@media only screen and (max-width:620px){.m-full{width:100%!important;}.m-pad{padding:16px!important; font-size:16px!important;}.m-hide{display:none!important;}.m-stack{display:block!important;width:100%!important;}.m-title{font-size:24px!important;line-height:30px!important;}}\n';
    h += '</style>\n\n</head>\n';
    ...
    h += '<p style="margin:0 0 14px; font-size:15px; color:rgba(255,255,255,0.85); font-weight:300; line-height:1.6; font-family:' + F + ';">' + escHtml(subtitle) + '</p>';
    h += '<p style="margin:0; font-size:13px; color:rgba(255,255,255,0.55); line-height:1.4; font-family:' + F + ';">' + escHtml(greeting) + '</p>';
    h += '</td></tr></table></td></tr>\n';
